...
首次部署成功后，回到项目 **设置** 页面，再按下面步骤创建并绑定存储。不要在首次构建尚未完成时判断 D1/KV 是否生效。
...
点击 **添加变量（Add variable）**，逐项填写名称和值；密码和三个加密密钥的类型选择 **密钥（Secret）**，填写后点击保存。推荐在绑定存储后完成下面表格中的 6 项，再执行第二次部署。
