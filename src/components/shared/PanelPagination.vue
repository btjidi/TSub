...
<template>
  ...
  >
    <div class="flex items-center space-x-4 text-xs text-gray-500 whitespace-nowrap">
      ...
      <div v-if="showItemsPerPage" class="flex items-center space-x-2">
        ...
        <select
          ...
          class="form-select tsub-radius-md text-xs py-1 pl-2 pr-6 border-gray-300 bg-gray-50 dark:bg-gray-900 dark:border-gray-600 focus:ring-primary-500/50 focus:border-primary-500"
        >
    ...
    <div v-if="totalPages > 1" class="flex items-center space-x-2">
      <button
        ...
        class="h-8 w-8 flex items-center justify-center tsub-radius-md bg-white/70 dark:bg-white/5 hover:bg-gray-100 dark:hover:bg-gray-700 disabled:opacity-30 disabled:hover:bg-transparent transition-colors text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 border border-gray-200/70 dark:border-white/10"
      ...
      <div class="flex items-center space-x-1">
        <input
          ...
          @blur="jumpToPage"
          class="w-14 text-center text-sm py-1 border border-gray-300 dark:border-gray-600 tsub-radius-md bg-gray-50 dark:bg-gray-900 focus:ring-1 focus:ring-primary-500/50 focus:border-primary-500 appearance-none remove-arrow"
        />
  ...
  <div v-else-if="shouldShow" class="flex flex-wrap justify-center items-center gap-3 mt-6 text-sm font-medium">
    <button
      @click="emitPageChange(currentPage - 1)"
      :disabled="currentPage === 1"
      class="h-8 px-3 tsub-radius-md disabled:opacity-50 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-700"
    >&laquo; {{ t('common.prevPage') }}</button>
    <span class="text-gray-500 dark:text-gray-400 bg-white/70 dark:bg-white/5 border border-gray-200/70 dark:border-white/10 px-3 py-1 tsub-radius-md">{{ t('common.pageOf', { current: currentPage, total: totalPages }) }}</span>
    <button
      @click="emitPageChange(currentPage + 1)"
      :disabled="currentPage === totalPages"
      class="h-8 px-3 tsub-radius-md disabled:opacity-50 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-700"
