...
const isDemo = computed(() => props.profile?.demo === true);
...
<template>
  ...
  >
    <div class="flex items-start justify-between gap-3" :class="compact ? 'flex-col' : ''">
      ...
      <div v-if="!isDemo" class="shrink-0 flex items-center gap-1 opacity-100 transition-opacity duration-200" :class="compact ? 'self-end flex-wrap justify-end' : ''">

		<button @click.stop="emit('preview')" class="p-2.5 rounded-full hover:bg-primary-50 dark:hover:bg-white/10 text-gray-400 hover:text-primary-500 transition-colors min-w-[44px] min-h-[44px] lg:min-w-0 lg:min-h-0 flex items-center justify-center" :title="t('actions.previewNodes')">
			<svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
		</button>
		<button @click.stop="emit('qrcode')" class="p-2.5 rounded-full hover:bg-primary-50 dark:hover:bg-white/10 text-gray-400 hover:text-primary-500 transition-colors min-w-[44px] min-h-[44px] lg:min-w-0 lg:min-h-0 flex items-center justify-center" :title="t('actions.showQrCode')">
		...
		<button @click.stop="emit('edit')" class="p-2.5 rounded-full hover:bg-primary-50 dark:hover:bg-white/10 text-gray-400 hover:text-primary-500 transition-colors min-w-[44px] min-h-[44px] lg:min-w-0 lg:min-h-0 flex items-center justify-center" :title="t('actions.edit')">
		...
		<button @click.stop="emit('delete')" class="p-2.5 rounded-full hover:bg-red-50 dark:hover:bg-red-500/20 text-gray-400 hover:text-red-500 transition-colors min-w-[44px] min-h-[44px] lg:min-w-0 lg:min-h-0 flex items-center justify-center" :title="t('actions.delete')">
    ...
    <div class="mt-4 grid gap-3 rounded-lg border border-gray-100 bg-gray-50/80 p-3 text-sm dark:border-white/10 dark:bg-white/5">
      <div class="flex items-center justify-between gap-3 text-gray-600 dark:text-gray-300">
        <span>{{ t('profiles.enabledStatus') }}</span>
        <Switch v-if="!isDemo"
          :model-value="isEnabled"
          @update:model-value="(val) => $emit('change', { ...profile, enabled: val })"
          :label="t('profiles.enable')"
        />
        <span v-else class="font-medium text-gray-800 dark:text-gray-200">{{ isEnabled ? t('common.enabled') : t('common.disabled') }}</span>
      </div>
      <div class="flex items-center justify-between gap-3 text-gray-600 dark:text-gray-300">
        <span>{{ t('profiles.publicAccess') }}</span>
        <Switch v-if="!isDemo"
          :model-value="isPublic"
          @update:model-value="(val) => $emit('change', { ...profile, isPublic: val })"
          :label="t('profiles.publicSwitch')"
        />
        <span v-else class="font-medium text-gray-800 dark:text-gray-200">{{ isPublic ? t('profiles.public') : t('profiles.private') }}</span>
      </div>
      <div class="flex items-center justify-between gap-3 text-xs text-gray-500 dark:text-gray-400">
        <span>{{ t('profiles.downloadCount', { count: profile.downloadCount || 0 }) }}</span>
        <button v-if="!isDemo" @click.stop="emit('view-logs')" class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-gray-600 transition-colors hover:bg-white hover:text-gray-800 dark:text-gray-300 dark:hover:bg-white/10 dark:hover:text-white" :title="t('actions.viewLogs')">
    ...
    <div class="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-gray-100 pt-3 dark:border-white/10" :class="compact ? 'items-stretch' : ''">
      <div class="flex items-center gap-2 sm:gap-4">
        <button v-if="!isDemo" @click="emit('open-copy')" class="inline-flex items-center gap-1.5 rounded-lg bg-indigo-50 px-4 py-2 text-sm font-medium text-indigo-600 transition-colors hover:bg-indigo-100 dark:bg-indigo-900/40 dark:text-indigo-300 dark:hover:bg-indigo-800/60" :class="compact ? 'w-full justify-center' : ''">
           ...
           {{ t('profiles.copySubscription') }}
         </button>
       </div>
      <div class="flex items-center gap-1 transition-opacity duration-200">
        <template v-if="isSorting && !isDemo">
          <button @click="emit('move-up')" class="p-1 rounded-full hover:bg-gray-500/10 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200" :title="t('actions.moveUp')">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M5 15l7-7 7 7" />
            </svg>
          </button>
          <button @click="emit('move-down')" class="p-1 rounded-full hover:bg-gray-500/10 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200" :title="t('actions.moveDown')">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
