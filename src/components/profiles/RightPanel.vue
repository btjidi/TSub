...
<template>
  <div>
    <div class="bg-white/90 dark:bg-gray-900/80 backdrop-blur-md tsub-radius-lg border border-gray-100/80 dark:border-white/10 shadow-sm transition-all duration-300" :class="compact ? 'p-4' : 'p-6'">
      ...
      <div v-if="hasSetupTasks" class="mb-4 rounded-[var(--tsub-radius-md)] border border-amber-200/80 bg-amber-50/80 p-3 text-amber-800 dark:border-amber-400/20 dark:bg-amber-500/10 dark:text-amber-200 list-item-animation" style="--delay-index: 1">
        ...
        <div class="mt-2 grid gap-2">
          <router-link
            ...
            :key="task.id"
            :to="task.to"
            class="block rounded-[var(--tsub-radius-sm)] border border-current/15 bg-white/50 px-3 py-2 text-xs hover:bg-white/80 dark:bg-white/10 dark:hover:bg-white/15 transition-colors"
          >
      ...
      <div class="mb-4 list-item-animation" style="--delay-index: 2">
        ...
        <select v-model="selectedId" class="w-full px-3 py-2.5 bg-white/80 dark:bg-gray-800/70 border border-gray-200/80 dark:border-white/10 tsub-radius-lg shadow-sm focus:outline-hidden focus:ring-2 focus:ring-primary-500/40 focus:border-primary-500 text-sm text-gray-900 dark:text-white input-enhanced">
      ...
      <div class="mb-5 list-item-animation" style="--delay-index: 3">
        ...
        <div class="grid gap-2" :class="compact ? 'grid-cols-2' : 'grid-cols-3'">
            <button
              ...
              class="min-h-11 px-3 py-2.5 text-xs font-medium tsub-radius-lg border transition-colors flex justify-center items-center text-center list-item-animation"
      ...
      <div class="relative list-item-animation" style="--delay-index: 4">
        <input
          ...
          class="w-full min-h-12 text-sm text-gray-600 dark:text-gray-300 bg-gray-100/80 dark:bg-gray-800/60 tsub-radius-lg pl-3 pr-24 py-3 border border-gray-200/70 dark:border-white/10 focus:outline-hidden focus:ring-2 font-mono input-enhanced"
          :class="{
        ...
        <div class="absolute right-1.5 top-1/2 -translate-y-1/2 flex items-center gap-1">
          <button @click="$emit('qrcode', subLink, t('dashboard.linkCard.qrTitle'))" :disabled="!isLinkValid" class="flex h-11 w-11 items-center justify-center tsub-radius-md text-gray-400 transition-colors duration-200" :class="isLinkValid ? 'hover:text-primary-600 hover:bg-white/80 dark:hover:bg-gray-800' : 'cursor-not-allowed'" :title="t('dashboard.linkCard.showQrCode')">
          ...
          <button @click="copyToClipboard" :disabled="!isLinkValid" class="flex h-11 w-11 items-center justify-center tsub-radius-md text-gray-400 transition-colors duration-200" :class="isLinkValid ? 'hover:text-primary-600 hover:bg-white/80 dark:hover:bg-gray-800' : 'cursor-not-allowed'" :title="t('dashboard.linkCard.copyLink')">
