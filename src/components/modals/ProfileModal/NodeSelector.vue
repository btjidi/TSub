...
<template>
  <div v-if="nodes.length > 0" class="space-y-2">
    ...
    <!-- Group Filter -->
    <div
      class="flex items-center gap-2 mb-2 p-1.5 tsub-radius-md border-b border-gray-100 dark:border-gray-700/50 overflow-x-auto no-scrollbar mask-gradient-r">
    ...
    <div class="relative mb-2">
      <input type="text" v-model="searchModel" :placeholder="t('manualNodes.searchPlaceholder')"
        class="w-full pl-9 pr-3 py-1.5 text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 tsub-radius-md shadow-xs focus:outline-hidden focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500" />
    ...
    <div
      class="overflow-y-auto space-y-2 p-3 bg-gray-50 dark:bg-gray-900/50 tsub-radius-md border dark:border-gray-700 h-36 lg:h-64">
    ...
    <div v-if="orderedSelectedNodes.length > 0" class="mt-3">
      ...
      <draggable v-model="orderedSelectedNodes" item-key="id" handle=".drag-handle" ghost-class="opacity-40"
        class="space-y-1 p-2 bg-indigo-50 dark:bg-indigo-900/20 tsub-radius-md border border-indigo-200 dark:border-indigo-800 h-32 lg:h-48 overflow-y-auto">
  ...
  <div v-else
    class="text-center text-sm text-gray-500 p-4 bg-gray-50 dark:bg-gray-900/50 tsub-radius-md flex items-center justify-center h-full">
