  ...
  >
    <template #title>
      <div class="flex items-center gap-3">
        <h3 class="text-lg font-bold text-gray-900 dark:text-white">{{ t('versionModal.title') }}</h3>
        <span 
          v-if="isLatest" 
        ...
        >
          {{ t('versionModal.available') }}
        </span>
      </div>
      <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
        <template v-if="isLatest">
          {{ t('versionModal.currentLatest', { version: currentVersion }) }}
        </template>
        <template v-else>
          {{ t('versionModal.updateFound', { latest: release.tag_name, current: currentVersion }) }}
        </template>
      </p>
    </template>
    ...
    <template #body>
      <div class="space-y-4">
        <div class="rounded-lg border border-indigo-200/70 bg-indigo-50/70 px-4 py-3 text-sm text-indigo-700 dark:border-indigo-500/20 dark:bg-indigo-500/10 dark:text-indigo-300">
          <div class="font-semibold">{{ isLatest ? t('versionModal.highlights', { version: currentVersion }) : (release.name || release.tag_name || t('versionModal.releaseNotes')) }}</div>
          <div v-if="!isLatest && release.published_at" class="mt-1 text-xs opacity-80">{{ t('versionModal.publishedAt', { time: new Date(release.published_at).toLocaleString(locale) }) }}</div>
        </div>
