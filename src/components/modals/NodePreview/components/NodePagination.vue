  ...
  <div v-if="totalPages > 1" class="shrink-0 border-t border-gray-200 bg-gray-50/80 p-4 dark:border-gray-700 dark:bg-gray-900/40 sm:p-5">
    ...
    <div v-if="currentPage > 0" class="hidden sm:flex items-center justify-between">
      <div class="text-sm text-gray-700 dark:text-gray-300">
        {{ t('nodePreview.itemRange', { start: ((currentPage - 1) * pageSize) + 1, end: Math.min(currentPage * pageSize, totalItems), total: totalItems }) }}
      </div>
    ...
    <div class="sm:hidden">
      <div class="mb-3 flex items-center justify-between">
        <div class="text-sm text-gray-700 dark:text-gray-300">
          {{ t('nodePreview.pageOf', { current: currentPage, total: totalPages }) }}
        </div>
        <div class="text-sm text-gray-500 dark:text-gray-400">
          {{ t('nodePreview.totalItems', { total: totalItems }) }}
        </div>
      </div>
