...
<template>
  <div class="min-h-0 flex-1 overflow-y-auto">
    ...
    <div class="block space-y-2 sm:space-y-3 lg:hidden">
      <div 
        v-for="(node, index) in nodes" 
        :key="`${node.url}_${index}`"
        @click="handleCardClick(node)"
        class="flex items-center justify-between rounded-xl border border-gray-100 bg-white p-3 shadow-sm transition-all active:scale-[0.98] dark:border-white/5 dark:bg-white/5 sm:rounded-2xl sm:p-4"
        :class="{ 
        ...
        <div class="flex flex-col h-full">
          ...
          <div class="mt-auto pt-4 border-t border-gray-100 dark:border-gray-700/50">
            <div class="flex items-center justify-between">
              <div class="min-w-0">
                <div class="text-[11px] text-gray-400 dark:text-gray-500 mb-0.5 font-medium uppercase tracking-tighter">{{ t('nodePreview.endpoint') }}</div>
