...
// Each virtual row is 56px high plus its 1px divider.
const itemHeight = 57;
...
<template>
  <div class="hidden lg:flex flex-1 overflow-hidden rounded-xl border border-gray-200/70 bg-white shadow-sm dark:border-white/10 dark:bg-white/5">
    <div ref="containerRef" data-testid="node-list-scroll" class="h-full w-full overflow-y-auto">
        ...
        <div class="w-full overflow-hidden rounded-xl border border-gray-200/70 dark:border-white/10">
          ...
          <div class="sticky top-0 z-10 border-b border-gray-100 bg-gray-50/50 backdrop-blur-md dark:border-white/5 dark:bg-gray-800/80">
            <div class="grid min-h-[3.5rem] grid-cols-12 gap-2 px-6 py-3 items-center text-[11px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest">
              <div v-if="selectionMode" class="col-span-1 flex justify-center text-indigo-500">{{ t('nodePreview.pick') }}</div>
              <div :class="selectionMode ? 'col-span-3' : 'col-span-4'">{{ t('nodePreview.nodeName') }}</div>
              <div class="col-span-3 hidden sm:block">{{ t('nodePreview.serverAddress') }}</div>
              <div class="col-span-2 hidden md:block text-center">{{ t('nodePreview.port') }}</div>
              <div class="col-span-1 hidden sm:block text-center">{{ t('nodePreview.type') }}</div>
              <div class="col-span-1 hidden sm:block text-center">{{ t('nodePreview.region') }}</div>
              <div class="col-span-1 text-center">{{ t('nodePreview.action') }}</div>
            </div>
          </div>
          ...
          <div class="bg-white dark:bg-gray-800" :style="{ height: totalHeight + 'px', position: 'relative' }">
            <div :style="{ transform: `translateY(${offsetY}px)` }">
              <div
                ...
                class="group h-[57px] border-b border-gray-50 transition-all hover:bg-gray-50/80 dark:border-white/5 dark:hover:bg-indigo-500/5"
