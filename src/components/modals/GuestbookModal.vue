            ...
            <div v-else class="space-y-4">
                <div>
                    ...
                    <select v-model="form.type"
                        class="block w-full tsub-radius-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 bg-white dark:bg-gray-700 dark:text-white py-2 px-3 text-sm">
                ...
                <div>
                    ...
                    <textarea v-model="form.content" rows="4" placeholder="请详细描述您的建议或遇到的问题..."
                        class="block w-full tsub-radius-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 bg-white dark:bg-gray-700 dark:text-white py-2 px-3 text-sm resize-none"></textarea>
                </div>
                ...
                <div>
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <input type="number" v-model="form.captcha" placeholder="答案"
                            class="block w-24 tsub-radius-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 bg-white dark:bg-gray-700 dark:text-white py-2 px-3 text-sm"
        ...
        <template #footer>
            <div class="flex w-full justify-end gap-3">
                <button @click="closeModal"
                    class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 tsub-radius-md hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-600 dark:hover:bg-gray-600 transition-colors">
                    {{ showSuccess ? '关闭' : '取消' }}
                </button>
                <button v-if="!showSuccess" @click="submitMessage" :disabled="submitting || !form.content"
                    class="px-4 py-2 text-sm font-medium text-white bg-indigo-600 border border-transparent tsub-radius-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-sm hover:shadow-md active:scale-95">
                    {{ submitting ? '提交中...' : '提交反馈' }}
                </button>
            </div>
        </template>
