...
<template>
  <!-- 包含/排除节点 -->
  <div class="border border-gray-200 dark:border-gray-700 tsub-radius-md overflow-hidden">
    <!-- 折叠头部 -->
    <div @click="emit('toggle-expand')"
      ...
      <div class="flex items-center gap-2">
        ...
        <label class="text-sm font-medium text-gray-700 dark:text-gray-300 cursor-pointer">{{ t('subscriptions.includeExclude') }}</label>
      ...
      <div class="flex items-center gap-2" @click.stop>
        <button v-if="!isAdvancedMode" @click="switchToAdvanced"
          ...
          {{ t('subscriptions.advancedMode') }}
        </button>
        <button v-else @click="switchToVisual"
          class="text-xs text-indigo-600 dark:text-indigo-400 hover:underline">
          {{ t('subscriptions.visualMode') }}
        </button>
      </div>
    </div>
      ...
      <div v-show="isRuleExpanded" class="px-4 py-3 border-t border-gray-200 dark:border-gray-700">
        ...
        <div v-if="!isAdvancedMode" class="space-y-3">
          ...
          <div class="flex gap-2">
            <button @click="ruleModeModel = 'exclude'" :class="[
              'flex-1 sm:flex-none px-3 py-1.5 text-xs sm:text-sm font-medium tsub-radius-md transition-all !min-h-0 !min-w-0',
              ruleModeModel === 'exclude'
                ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-200'
                : 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600'
            ]">
              {{ t('subscriptions.excludeMode') }}
            </button>
            <button @click="ruleModeModel = 'keep'" :class="[
              ...
              {{ t('subscriptions.keepMode') }}
            </button>
          </div>

          <!-- 地区标签 -->
          <div>
            <div class="text-xs text-gray-500 dark:text-gray-400 mb-1.5">📍 {{ t('subscriptions.region') }}</div>
            <div class="flex flex-wrap gap-1.5">
              <button v-for="tag in presetRegions" :key="tag.pattern" @click="toggleTag(tag, 'region')" :class="[
                'px-2.5 py-1 text-xs sm:text-sm font-medium tsub-radius-md transition-all !min-h-0 !min-w-0',
          ...
          <!-- 协议标签 -->
          <div>
            <div class="text-xs text-gray-500 dark:text-gray-400 mb-1.5">📡 {{ t('subscriptions.protocol') }}</div>
            <div class="flex flex-wrap gap-1.5">
          ...
          <!-- 关键词标签 -->
          <div>
            <div class="text-xs text-gray-500 dark:text-gray-400 mb-1.5">🏷️ {{ t('subscriptions.keyword') }}</div>
            <div class="flex flex-wrap gap-1.5">
          ...
          <!-- 自定义关键字输入 -->
          <div>
            <div class="text-xs text-gray-500 dark:text-gray-400 mb-1.5">✏️ {{ t('subscriptions.customKeyword') }}</div>
            <div class="flex flex-col sm:flex-row gap-2">
              <input type="text" v-model="customKeywordModel" @keyup.enter="addCustomKeyword" :placeholder="t('subscriptions.keywordPlaceholder')"
                class="w-full sm:flex-1 min-w-0 px-3 py-1.5 text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 tsub-radius-md focus:outline-hidden focus:ring-1 focus:ring-indigo-500 dark:text-white">
              <button @click="addCustomKeyword"
                class="w-full sm:w-auto flex-shrink-0 whitespace-nowrap px-3 py-1.5 text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 tsub-radius-md transition-colors !min-h-0 !min-w-0">
                {{ t('subscriptions.addKeyword') }}
              </button>
            </div>
          </div>
          ...
          <div v-if="selectedRules.length > 0">
            <div class="text-xs text-gray-500 dark:text-gray-400 mb-1.5">
              📋 {{ t('subscriptions.selectedRules', { count: selectedRules.length }) }}
              <span class="ml-1" :class="ruleModeModel === 'keep' ? 'text-emerald-600' : 'text-amber-600'">
                - {{ ruleModeModel === 'keep' ? t('subscriptions.keepMatches') : t('subscriptions.excludeMatches') }}
              </span>
            </div>
            <div class="flex flex-wrap gap-1.5 p-2 bg-gray-50 dark:bg-gray-800/50 tsub-radius-md">
              <span v-for="(rule, index) in selectedRules" :key="index" :class="[
                'inline-flex items-center gap-1 px-2.5 py-1 text-xs sm:text-sm font-medium tsub-radius-md !min-h-0 !min-w-0',
          ...
          <p class="text-xs text-gray-500 dark:text-gray-400">
            {{ ruleModeModel === 'keep' ? t('subscriptions.keepHint') : t('subscriptions.excludeHint') }}
          </p>
        </div>
        ...
        <div v-else>
          ...
          <textarea id="sub-edit-exclude" v-model="editingSubscription.exclude"
            placeholder="[排除模式(默认)]&#10;proto:vless,trojan&#10;(过期|官网)&#10;---&#10;[包含模式(只保留匹配项)]&#10;keep:(香港|HK)&#10;keep:proto:ss"
            rows="8" :class="[
              'w-full px-3 py-2 tsub-radius-md sm:text-sm font-mono dark:text-white leading-6 resize-none bg-white dark:bg-gray-800',
          ...
          <div v-if="excludeRuleState.errors.length" class="mt-2 text-xs text-red-600 dark:text-red-400">
            {{ t('subscriptions.ruleInvalid', { message: excludeRuleState.errorsText }) }}
          </div>
          <div class="mt-2 text-xs text-gray-500 dark:text-gray-400 space-y-1">
            <p>{{ t('subscriptions.ruleLineHelp') }} <code class="font-mono">proto:</code></p>
            <p>{{ t('subscriptions.ruleKeepHelp') }}</p>
          </div>
        </div>
      </div>
    </Transition>
