...
const { t } = useI18n();
const isManagedSubscription = computed(() => ['tsub-deployment-push', 'tsub-deployment-snapshot']
  .includes(props.editingSubscription?.source?.kind));
  ...
  <Modal v-if="editingSubscription" :show="show" size="2xl"
    ...
    <template #title>
      <div class="flex items-center gap-3">
        <div class="p-2 tsub-radius-lg bg-indigo-500/10">
        ...
        <div>
          ...
          <p class="mt-0.5 text-sm text-gray-500 dark:text-gray-400">{{ t(isManagedSubscription ? 'subscriptions.managedModalDescription' : 'subscriptions.modalDescription') }}</p>
        </div>
      </div>
    </template>
    <template #body>
      <div class="space-y-4">
        <EditForm :editing-subscription="editingSubscription" :managed="isManagedSubscription" />
        ...
        <AdvancedOptions :editing-subscription="editingSubscription" :managed="isManagedSubscription" />
      </div>
    </template>
  </Modal>
</template>
