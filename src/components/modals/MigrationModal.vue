...
const handleMigrate = async () => {
    ...
    try {
        ...
        addLog('提示：若仍失败，请确认 TSUB_DB 绑定与 D1 表创建权限。', 'warning');
    ...
    <template #body>
       <div class="h-[400px] flex flex-col">
           ...
           <div v-if="step === 'check'" class="flex-1 space-y-4">
               <div class="bg-blue-50 dark:bg-blue-900/20 p-4 tsub-radius-md border border-blue-100 dark:border-blue-800">
                   ...
                   <ul class="space-y-3 text-sm text-gray-700 dark:text-gray-300">
                       ...
                       <li class="flex items-start gap-2">
                           <input type="checkbox" checked disabled class="mt-1 h-4 w-4 text-indigo-600 rounded border-gray-300">
                           <span>在 Pages 设置中绑定 D1 数据库变量为 <code>TSUB_DB</code></span>
                       </li>
                       <li class="flex items-start gap-2">
           ...
           <div v-else class="flex-1 flex flex-col min-h-0">
               <div class="bg-gray-900 tsub-radius-md p-4 font-mono text-xs overflow-y-auto flex-1 custom-scrollbar shadow-inner border border-gray-700">
