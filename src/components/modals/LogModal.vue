             ...
             <div class="flex justify-between items-center w-full">
                <span class="text-lg font-bold text-gray-900 dark:text-white">{{ t('logs.title') }}</span>
                 <div class="flex gap-2">
                    <button 
                    @click="fetchLogs" 
                    class="text-xs px-2 py-1 tsub-radius-md bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors text-gray-600 dark:text-gray-300">
                        {{ t('common.refresh') }}
                    </button>
                    <button 
                    @click="clearLogs" 
                    class="text-xs px-2 py-1 tsub-radius-md bg-red-50 dark:bg-red-900/30 hover:bg-red-100 dark:hover:bg-red-900/50 transition-colors text-red-600 dark:text-red-400">
                        {{ t('common.clear') }}
                    </button>
                </div>
            </div>
             ...
             <div class="flex flex-col h-[60vh]">
                <div class="flex-1 overflow-y-auto custom-scrollbar pr-2 min-h-0">
                    ...
                    <ul v-else class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <li v-for="log in paginatedLogs" :key="log.id || log.timestamp" class="bg-gray-50 dark:bg-gray-700/30 tsub-radius-md p-2.5 text-xs border border-gray-100 dark:border-gray-700 relative overflow-hidden group hover:border-indigo-200 dark:hover:border-indigo-800 transition-colors cursor-pointer" @click="toggleExpand(log.id)">
