...
const clients = computed(() => [
  { name: t('copyLinkModal.auto'), type: 'default', icon: 'M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1', format: '' },
...
const copyToClipboard = async (format) => {
  if (!baseUrl.value) {
    showToast(t('copyLinkModal.tokenRequired'), 'error');
    return;
  }
...
const fallbackCopy = (link) => {
  ...
  try {
    ...
    if (successful) {
      ...
      showToast(t('copyLinkModal.copyFailed'), 'error');
    }
  } catch (err) {
    showToast(t('copyLinkModal.copyFailed'), 'error');
  }
...
<template>
  <Modal :show="show" @update:show="close" :show-cancel="false" :show-confirm="false">
    <template #title>
      <h3 class="text-xl font-bold text-gray-900 dark:text-white">{{ t('copyLinkModal.title') }}</h3>
    </template>
    ...
    <template #body>
      <div class="mt-2" v-if="!baseUrl">
         <p class="text-red-500 text-sm bg-red-50 dark:bg-red-900/20 p-3 rounded-lg border border-red-200 dark:border-red-800">
           {{ t('copyLinkModal.missingToken') }}
         </p>
      </div>
      <div v-else class="space-y-3 mt-4">
        <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">
          {{ t('copyLinkModal.hint') }}
        </p>
