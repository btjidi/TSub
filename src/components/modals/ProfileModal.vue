...
const toggleSelection = (listName, id) => {
  ...
  const index = list.findIndex(item => (item && typeof item === 'object' ? item.id : item) === id);
  if (index > -1) {
    list.splice(index, 1);
  } else {
    list.push(id);
...
const handleSelectAll = (listName, sourceArray) => {
  ...
  const retained = localProfile.value[listName].filter(item => !sourceIds.has(item && typeof item === 'object' ? item.id : item));
  localProfile.value[listName] = [...retained, ...sourceArray.map(item => item.id)];
};

const handleDeselectAll = (listName, sourceArray) => {
  const sourceIds = sourceArray.map(item => item.id);
  localProfile.value[listName] = localProfile.value[listName].filter(item => !sourceIds.includes(item && typeof item === 'object' ? item.id : item));
};
