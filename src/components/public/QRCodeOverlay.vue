...
<template>
  <Teleport to="body">
    <Transition name="qr-overlay">
      ...
      >
        <div class="relative bg-white dark:bg-gray-800 tsub-radius-lg p-8 shadow-2xl max-w-sm w-full mx-4 transform transition-all border border-gray-100 dark:border-gray-700"
          @click.stop>
          <!-- Close Button -->
          <button
            @click="emit('close')"
          ...
          <div class="flex flex-col items-center pt-2">
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mb-6">{{ t('publicProfiles.scanQr') }}</p>

            <div class="bg-white p-3 tsub-radius-lg shadow-inner border border-gray-100 w-64 h-64 flex items-center justify-center">
              <canvas
                :ref="el => { if (el) emit('register-canvas', profile.id, el); }"
                class="max-w-full h-auto tsub-radius-md"
              ></canvas>
            </div>

            <button
              @click="emit('download')"
              class="mt-8 w-full flex items-center justify-center gap-2 px-6 py-3.5 text-sm font-bold text-white bg-primary-600 hover:bg-primary-700 tsub-radius-lg transition-all shadow-lg shadow-primary-600/20 active:scale-95"
            >
              <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                  d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
              </svg>
              {{ t('publicProfiles.downloadQr') }}
            </button>
          </div>
        </div>
      </div>
