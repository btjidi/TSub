    ...
    <div v-if="isDirty || saveState === 'success'"
      ...
      <div class="flex items-center gap-3 w-full sm:w-auto">
        <button v-if="saveState !== 'success'" @click="$emit('discard')"
          class="flex-1 sm:flex-none px-4 py-2 text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors hover:bg-black/5 dark:hover:bg-white/5 tsub-radius-md whitespace-nowrap">
          {{ t('actions.discard') }}
        </button>
        <button @click.prevent="$emit('save')" :disabled="saveState !== 'idle'"
          class="flex-1 sm:flex-none px-6 py-2 text-sm text-white font-bold tsub-radius-lg flex items-center justify-center transition-all duration-300 transform active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed whitespace-nowrap"
          :class="{ 
