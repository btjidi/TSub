...
const sizeMap = {
  sm: {
    input: 'py-1.5 text-xs tsub-radius-md px-2',
    label: 'text-xs mb-1 ml-1',
    icon: 'h-4 w-4'
  },
  md: {
    input: 'py-2 text-sm tsub-radius-md px-3',
    label: 'text-sm mb-1.5 ml-1',
    icon: 'h-4 w-4'
  },
  lg: {
    input: 'py-3 text-base tsub-radius-lg px-4',
    label: 'text-sm mb-1.5 ml-1',
    icon: 'h-5 w-5'
  }
};
...
<template>
  <div class="w-full">
    ...
    <div class="relative group">
      ...
      <!-- Focus Glow -->
      <div class="absolute inset-0 tsub-radius-md pointer-events-none transition-opacity duration-150 opacity-0 group-focus-within:opacity-100 ring-1 ring-primary-500/20"></div>
    </div>
