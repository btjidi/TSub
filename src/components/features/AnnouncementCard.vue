...
<template>
    <Transition name="fade-slide">
        <div v-if="isVisible" 
             class="announcement-glass group relative overflow-hidden transition-all duration-500 tsub-radius-lg border backdrop-blur-xl shadow-lg"
            ...
            <div class="relative p-5 sm:p-6 z-10 flex flex-col sm:flex-row gap-5">
                ...
                <div class="flex-1 space-y-1.5 min-w-0">
                    ...
                    <div v-if="announcement.updatedAt" class="pt-2 text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest flex items-center gap-2">
                        ...
                        {{ t('common.updatedAt', { time: new Date(announcement.updatedAt).toLocaleString(locale.value) }) }}
                    </div>
                </div>
            </div>
        </div>
