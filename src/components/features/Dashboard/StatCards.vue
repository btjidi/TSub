...
const cardBaseClass = 'stat-card group w-full text-left bg-white/90 dark:bg-gray-900/70 p-4 tsub-radius-lg shadow-sm border transition-all duration-500 hover:-translate-y-0.5 hover:shadow-md focus:outline-hidden focus:ring-2 focus:ring-primary-500/30';
  ...
  <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
    ...
    >
      <div class="flex items-center justify-between mb-2">
        ...
        <span class="p-1.5 tsub-radius-md" :class="hasTrafficWarning ? 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-300' : 'bg-green-50 text-green-600 dark:bg-green-900/30 dark:text-green-400'">
    ...
    <button
      ...
      style="--delay: 100ms"
      @click="navigate('/dashboard/groups', hasSubscriptionWarning ? { status: subscriptionsCount === 0 ? 'missing' : 'disabled' } : {})"
    >
      <div class="flex items-center justify-between mb-2">
        <h3 class="text-xs font-medium text-gray-500 uppercase">{{ t('dashboard.activeSubscriptions') }}</h3>
        <span class="p-1.5 tsub-radius-md" :class="hasSubscriptionWarning ? 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-300' : 'bg-indigo-50 text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-400'">
    ...
    >
      <div class="flex items-center justify-between mb-2">
        ...
        <span class="p-1.5 tsub-radius-md" :class="hasNodeWarning ? 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-300' : 'bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400'">
    ...
    >
      <div class="flex items-center justify-between mb-2">
        ...
        <span class="p-1.5 tsub-radius-md" :class="hasProfileWarning ? 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-300' : 'bg-purple-50 text-purple-600 dark:bg-purple-900/30 dark:text-purple-400'">
