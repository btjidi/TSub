...
const clearPassword = () => {
  ...
  if (!props.settings.secretActions.clearPaths.includes('webdavBackup.password')) props.settings.secretActions.clearPaths.push('webdavBackup.password');
  ensureConfig().password = '';
};
  ...
  <div class="rounded-xl border border-gray-100/80 bg-white/90 p-6 shadow-sm dark:border-white/10 dark:bg-gray-900/70">
    ...
    <div class="space-y-5">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Input :label="t('settings.webdavRemotePath')" v-model="settings.webdavBackup.remotePath" placeholder="/TSub" />
        ...
        <div v-if="settings.secretStatus?.['webdavBackup.password'] && !settings.webdavBackup.password" class="flex items-center justify-between gap-3 text-xs text-emerald-700 dark:text-emerald-300">
          <span>{{ t('settings.secretConfigured') }}</span>
          <button type="button" @click="clearPassword" class="text-red-600 hover:text-red-700 dark:text-red-400">{{ t('settings.clearSecret') }}</button>
        </div>
        <Input :label="t('settings.webdavFilenameTemplate')" v-model="settings.webdavBackup.filenameTemplate" placeholder="tsub-backup-{datetime}.json" />
