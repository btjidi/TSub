...
const isConfigured = computed(() => Boolean(props.settings.secretStatus?.cronSecret));

function clearCronSecret() {
    ...
    if (!props.settings.secretActions.clearPaths.includes('cronSecret')) props.settings.secretActions.clearPaths.push('cronSecret');
    props.settings.cronSecret = '';
}
...
<template>
    ...
    <div
        class="bg-white/90 dark:bg-gray-900/70 tsub-radius-lg p-6 space-y-5 border border-gray-100/80 dark:border-white/10 shadow-sm transition-shadow duration-300">
        ...
        <div>
            ...
            <input type="text" v-model="settings.cronSecret" :placeholder="t('settings.cronSecretPlaceholder')"
                class="block w-full px-3 py-2.5 bg-white/70 dark:bg-gray-900/50 border border-gray-200/80 dark:border-white/10 tsub-radius-lg shadow-sm focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500 sm:text-sm dark:text-white transition-colors">
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                {{ t('settings.cronSecretHint') }}
            </p>
            <div v-if="isConfigured && !settings.cronSecret" class="mt-2 flex items-center justify-between gap-3 text-xs">
                <span class="text-emerald-700 dark:text-emerald-300">{{ t('settings.secretConfigured') }}</span>
                <button type="button" @click="clearCronSecret" class="text-red-600 hover:text-red-700 dark:text-red-400">{{ t('settings.clearSecret') }}</button>
            </div>
        </div>
        ...
        <div v-if="cronUrl" class="space-y-3">
            <div>
                ...
                <div class="flex tsub-radius-lg shadow-xs">
            ...
            <div class="bg-blue-50/70 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800/60 tsub-radius-md p-3 space-y-2">
        ...
        <div v-else
            class="bg-yellow-50/80 dark:bg-yellow-900/20 border border-yellow-200/80 dark:border-yellow-800/70 tsub-radius-md p-3">
