...
const legacyBotConfigured = computed(() => Boolean(props.settings.secretStatus?.BotToken));
const pushBotConfigured = computed(() => Boolean(props.settings.secretStatus?.['telegram_push_config.bot_token']));
const webhookSecretConfigured = computed(() => Boolean(props.settings.secretStatus?.['telegram_push_config.webhook_secret']));
...
function clearSecret(path) {
  ...
  if (!props.settings.secretActions.clearPaths.includes(path)) props.settings.secretActions.clearPaths.push(path);
  if (path === 'BotToken') props.settings.BotToken = '';
  if (path === 'telegram_push_config.bot_token') telegramPushConfig.value.bot_token = '';
  if (path === 'telegram_push_config.webhook_secret') telegramPushConfig.value.webhook_secret = '';
}
...
<template>
  <div class="space-y-6">
    <section class="bg-white/90 dark:bg-gray-900/70 tsub-radius-lg p-6 border border-gray-100/80 dark:border-white/10 shadow-sm space-y-4">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <input
            ...
            class="block w-full px-3 py-2 bg-gray-50 dark:bg-gray-900/50 border border-gray-300 dark:border-gray-600 tsub-radius-lg shadow-xs focus:ring-1 focus:ring-blue-500 focus:border-blue-500 sm:text-sm dark:text-white transition-colors"
          >
          <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ t('settings.telegramBotTokenHint') }}</p>
          <div v-if="legacyBotConfigured && !settings.BotToken" class="mt-2 flex items-center justify-between gap-3 text-xs">
            <span class="text-emerald-700 dark:text-emerald-300">{{ t('settings.secretConfigured') }}</span>
            <button type="button" @click="clearSecret('BotToken')" class="text-red-600 hover:text-red-700 dark:text-red-400">{{ t('settings.clearSecret') }}</button>
          </div>
        </div>
        <div>
      ...
      <div class="flex flex-col sm:flex-row sm:items-center gap-3">
        <button
          type="button"
          class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium tsub-radius-lg disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2"
          :disabled="isTesting || (!settings.BotToken && !legacyBotConfigured) || !settings.ChatID"
    ...
    <section class="bg-white/90 dark:bg-gray-900/70 tsub-radius-lg p-6 border border-gray-100/80 dark:border-white/10 shadow-sm space-y-6">
      <div class="flex items-center justify-between gap-4">
        <div>
      ...
      <div v-if="telegramPushConfig.enabled" class="space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <input
              ...
              type="password"
              placeholder="123456:ABC-DEF..."
              class="mt-1 block w-full px-3 py-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 tsub-radius-md shadow-xs focus:outline-hidden focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:text-white"
            >
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ t('settings.telegramPushTokenHint') }}</p>
            <div v-if="pushBotConfigured && !telegramPushConfig.bot_token" class="mt-2 flex items-center justify-between gap-3 text-xs">
              <span class="text-emerald-700 dark:text-emerald-300">{{ t('settings.secretConfigured') }}</span>
              <button type="button" @click="clearSecret('telegram_push_config.bot_token')" class="text-red-600 hover:text-red-700 dark:text-red-400">{{ t('settings.clearSecret') }}</button>
            </div>
          </div>
          ...
          <div>
            ...
            <div v-if="webhookSecretConfigured && !telegramPushConfig.webhook_secret" class="mt-2 flex items-center justify-between gap-3 text-xs">
              <span class="text-emerald-700 dark:text-emerald-300">{{ t('settings.secretConfigured') }}</span>
              <button type="button" @click="clearSecret('telegram_push_config.webhook_secret')" class="text-red-600 hover:text-red-700 dark:text-red-400">{{ t('settings.clearSecret') }}</button>
            </div>
        ...
        <div class="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 tsub-radius-md p-4">
        ...
        <div>
          ...
          <div class="mt-1 flex tsub-radius-md shadow-xs">
            <input
              :value="webhookUrl"
              type="text"
              readonly
        ...
        <div v-else class="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 tsub-radius-md p-3">
        ...
        <div class="flex flex-col sm:flex-row gap-2">
          <button
            type="button"
            class="flex items-center justify-center sm:justify-start gap-2 px-4 py-2 text-sm font-medium tsub-radius-md border transition-colors flex-1"
        ...
        <div v-if="showSetupGuide" class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 tsub-radius-md p-4">
        ...
        <div v-if="showUsageGuide" class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 tsub-radius-md p-4">
