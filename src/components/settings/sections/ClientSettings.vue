    ...
    <div class="space-y-6">
        <div class="rounded-xl border border-gray-100/80 bg-white/90 p-6 shadow-sm dark:border-white/10 dark:bg-gray-900/70">
            ...
            <div class="flex gap-2 w-full md:w-auto">
                <button @click="handleInit"
                    class="flex-1 md:flex-none px-3 py-1.5 text-center text-sm text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 tsub-radius-lg border border-gray-300 dark:border-gray-600">{{ t('settings.clientResetDefault') }}</button>
                <button @click="handleAdd"
                    class="flex-1 md:flex-none px-4 py-2 text-center text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 tsub-radius-lg shadow-sm shadow-primary-500/20">{{ t('settings.clientAdd') }}</button>
            </div>
        </div>
        </div>
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div v-for="(client, index) in clients" :key="client.id"
                class="relative bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 tsub-radius-lg p-4 hover:shadow-md transition-shadow group">

                <div class="flex items-start gap-3 min-w-0">
                    <div
                        class="h-12 w-12 tsub-radius-lg flex items-center justify-center text-2xl shrink-0 bg-gray-50 dark:bg-gray-700/50 text-gray-600 overflow-hidden ring-1 ring-gray-200/60 dark:ring-white/10">
                ...
                <div class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-8">
                        ...
                        <div class="md:col-span-5 lg:col-span-4">
                            <div
                                class="bg-gray-50 dark:bg-gray-700/30 tsub-radius-lg p-6 flex flex-col items-center gap-8 h-full border border-gray-100 dark:border-gray-700/50">
                                <div class="text-center space-y-3 w-full">
                                    ...
                                    <div
                                        class="aspect-square w-full max-w-[180px] mx-auto tsub-radius-lg flex items-center justify-center text-6xl bg-white dark:bg-gray-800 text-gray-600 border-2 border-dashed border-gray-200 dark:border-gray-600 overflow-hidden shadow-sm">
                                ...
                                <div class="w-full space-y-4 mt-2">
                                    <div class="space-y-2">
                                        ...
                                        <div class="flex gap-2 justify-center">
                                            <button @click="triggerIconFileSelect" type="button"
                                                class="flex items-center gap-1.5 px-3 py-1.5 text-sm text-gray-600 dark:text-gray-300 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 tsub-radius-md hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors"
                                            ...
                                            <button v-if="editingClient.icon" @click="clearIcon" type="button"
                                                class="flex items-center gap-1.5 px-3 py-1.5 text-sm text-gray-600 dark:text-gray-300 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 tsub-radius-md hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/30 dark:hover:text-red-400 transition-colors">
                                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <div class="relative tsub-radius-md shadow-sm">
                                            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                                <span class="text-gray-500 sm:text-sm">🔗</span>
                                            </div>
                                            <input v-model="iconInputValue" type="text" :placeholder="t('settings.clientIconPlaceholder')"
                                                class="block w-full pl-10 pr-3 tsub-radius-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-2.5">
                                        </div>
                        ...
                        <div class="md:col-span-7 lg:col-span-8 flex flex-col gap-6">
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <input v-model="editingClient.name" type="text"
                                        class="mt-1 block w-full tsub-radius-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm px-4 py-2.5 text-lg font-medium"
                                ...
                                <div>
                                    ...
                                    <textarea v-model="editingClient.description" rows="3"
                                        class="mt-1 block w-full tsub-radius-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm px-4 py-3 resize-none"
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                                    <label v-for="opt in platformOptions" :key="opt.value"
                                        class="cursor-pointer group relative flex flex-col items-center justify-center p-3 tsub-radius-lg border transition-all duration-200 select-none text-center"
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 pt-2">
                                <div>
                                    ...
                                    <div class="mt-1 flex tsub-radius-md shadow-sm">
                                        <span
            ...
            <template #footer>
                <div class="flex justify-end gap-3">
                    <button @click="showEditModal = false"
                        class="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 tsub-radius-md hover:bg-gray-50 dark:hover:bg-gray-700">{{ t('actions.cancel') }}</button>
                    <button @click="handleSave" :disabled="saving"
                        class="px-4 py-2 text-sm font-medium text-white bg-indigo-600 border border-transparent tsub-radius-md hover:bg-indigo-700 focus:outline-hidden focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50">
        ...
        <Modal v-model:show="showResetConfirm" @confirm="executeReset" :title="t('settings.clientResetConfirmTitle')" :confirmText="t('settings.clientResetConfirmAction')" :cancelText="t('actions.cancel')"
            ...
            <template #body>
                <div class="space-y-3">
                    <div
                        class="flex items-center gap-3 text-amber-600 dark:text-amber-500 bg-amber-50 dark:bg-amber-900/20 p-3 tsub-radius-md border border-amber-100 dark:border-amber-900/30">
        ...
        <Modal v-model:show="showDeleteConfirm" @confirm="executeDelete" :title="t('settings.clientDeleteConfirmTitle')" :confirmText="t('settings.clientDeleteConfirmAction')" :cancelText="t('actions.cancel')"
            ...
            <template #body>
                <div class="space-y-3">
                    <div
                        class="flex items-center gap-3 text-red-600 dark:text-red-500 bg-red-50 dark:bg-red-900/20 p-3 tsub-radius-md border border-red-100 dark:border-red-900/30">
                        <svg class="w-6 h-6 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                        </svg>
