...
const isDemo = computed(() => props.node?.demo === true);
...
<template>
  <div 
    class="group relative flex min-h-[88px] items-center justify-between gap-3 rounded-xl border border-gray-100 bg-white p-4 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-primary-500/5 dark:border-white/10 dark:bg-gray-900/70"
    :class="{ 
        'opacity-50 grayscale': !node.enabled && !isSelectionMode,
        'ring-2 ring-primary-500 bg-primary-50 dark:bg-primary-900/10': isSelectionMode && !isDemo && isSelected,
        'cursor-pointer': isSelectionMode && !isDemo
    }"
    @click="isSelectionMode && !isDemo ? emit('toggle-select') : null"
  >
    <!-- Selection Checkbox -->
    <div v-if="isSelectionMode && !isDemo" class="shrink-0 mr-1">
    ...
    <div v-if="!isSelectionMode && !isDemo" class="shrink-0 flex items-center gap-1 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-200">
