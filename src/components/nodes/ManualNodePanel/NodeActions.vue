...
<template>
  <div class="mb-4 rounded-xl border border-gray-100/80 bg-white/85 p-4 shadow-sm dark:border-white/10 dark:bg-gray-900/70">
      ...
      <!-- Mobile Group Filter -->
       <div class="flex md:hidden items-center overflow-x-auto no-scrollbar gap-2 py-1 max-w-full">
         <button 
           ...
           class="px-2.5 py-1 text-xs font-medium tsub-radius-md transition-all border shrink-0 whitespace-nowrap"
    ...
    <div class="flex items-center gap-2 w-full sm:w-auto">
      ...
      <div class="p-0.5 bg-gray-200 dark:bg-gray-700 tsub-radius-md flex items-center shrink-0">
        <button @click="emit('update:viewMode', 'card')" class="view-mode-toggle p-1.5 tsub-radius-sm transition-colors flex items-center justify-center" :class="viewMode === 'card' ? 'bg-white dark:bg-gray-900 text-indigo-600' : 'text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white'">
        ...
        <button @click="emit('update:viewMode', 'list')" class="view-mode-toggle p-1.5 tsub-radius-sm transition-colors flex items-center justify-center" :class="viewMode === 'list' ? 'bg-white dark:bg-gray-900 text-indigo-600' : 'text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white'">
  ...
  <div class="hidden md:flex items-center gap-2 mt-3 pt-3 border-t border-gray-100/80 dark:border-white/10 overflow-x-auto no-scrollbar mask-gradient-r pb-1">
    <button 
      @click="emit('set-group-filter', null)"
      class="px-3 py-1 text-xs font-medium tsub-radius-md transition-all border shrink-0"
