...
import { isRouteNavigationPending, pendingRoutePath } from '../../state/routeNavigation.js';
...
function isActive(path) {
  const currentPath = isRouteNavigationPending.value && pendingRoutePath.value
...
<template>
  <header
    ...
    class="app-nav-bar lg:hidden sticky top-0 z-50 flex items-center justify-between px-4 py-2.5 w-full bg-white/90 dark:bg-[#030712]/88 backdrop-blur-xl border-b border-gray-200/60 dark:border-white/10 shadow-sm transition-all duration-300"
  >
  ...
  <header
    ...
    class="app-nav-bar hidden lg:block sticky top-0 z-50 w-full bg-white/90 dark:bg-[#030712]/88 backdrop-blur-xl border-b border-gray-200/60 dark:border-white/10 transition-all duration-300"
  >
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 md:h-[76px] flex items-center justify-between">
      <div class="flex min-w-0 items-center gap-4">
        ...
        <nav :aria-label="t('nav.main')" class="nav-tab-shell min-w-0">
        <router-link
  ...
  <nav
    ...
    class="lg:hidden mobile-nav-glass z-[60]"
  >
