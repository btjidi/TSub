...
const props = defineProps({
  ...
  totalPages: Number,
  isSorting: Boolean,
  enableFileImport: { type: Boolean, default: false },
});

const emit = defineEmits(['add', 'delete', 'changePage', 'updateNodeCount', 'edit', 'toggleSort', 'markDirty', 'preview', 'deleteAll', 'refreshAll', 'reorder', 'import', 'importFile', 'qrcode', 'history']);
const fileInput = ref(null);

const draggableSubscriptions = computed({
    get: () => props.subscriptions.filter(subscription => subscription?.demo !== true),
...
const handleHistory = (id) => emit('history', id);
  ...
  <div>
    <div class="mb-4 rounded-xl border border-gray-100/80 bg-white/85 p-4 shadow-sm dark:border-white/10 dark:bg-gray-900/70">
      <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div class="flex flex-wrap items-center gap-2 sm:w-auto justify-end sm:justify-start">
          ...
          <input ref="fileInput" data-testid="subscription-file-input" class="sr-only" type="file" accept=".txt,.csv,.json,text/plain,text/csv,application/json" @change="handleFileImport" />
          ...
          <button v-if="enableFileImport" data-testid="subscription-file-import" type="button" @click="fileInput?.click()" class="inline-flex shrink-0 items-center rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-primary-500/30 dark:border-white/10 dark:bg-white/5 dark:text-gray-200 dark:hover:bg-white/10" :title="t('subscriptions.fileImportHint')">{{ t('actions.fileImport') }}</button>
        ...
        <template #item="{ element: subscription }">
          <div class="cursor-move">
              <Card
                  ...
                  @history="handleHistory(subscription.id)" />
          </div>
        </template>
      </draggable>
    ...
    <div v-else class="rounded-xl border border-dashed border-gray-300 bg-white/60 py-6 dark:border-gray-700 dark:bg-gray-900/50">
      ...
      <div class="-mt-8 mb-8 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
        ...
        <button v-if="enableFileImport" data-testid="empty-file-import-subscriptions" @click="fileInput?.click()" class="inline-flex items-center justify-center rounded-lg border border-gray-200 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-primary-500/30 dark:border-white/10 dark:bg-white/5 dark:text-gray-300 dark:hover:bg-white/10">{{ t('actions.fileImport') }}</button>
      </div>
    </div>
  </div>
</template>
