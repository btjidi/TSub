...
function resolveMonitoringEndpoint() {
  if (typeof window !== 'undefined' && window.__TSUB_ERROR_REPORT_URL__) {
    return window.__TSUB_ERROR_REPORT_URL__;
  }
...
class ErrorHandler {
  ...
  getUserFriendlyMessage(errorInfo) {
    ...
    if (message.includes('TSUB_KV') || message.includes('KV 绑定')) {
      return '服务端存储未初始化，请联系管理员配置 KV 绑定';
    }
    if (message.includes('TSUB_DB') || message.includes('D1 绑定')) {
