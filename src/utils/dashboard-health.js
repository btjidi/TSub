...
function getTrafficState(sub, now) {
  ...
  if (!sub?.enabled || !hasTrafficUsage(info)) return null;

  const total = resolveEffectiveTrafficTotal(sub, info);
...
export function getDashboardHealthItems({
  ...
  if (subscriptions.length === 0) {
    items.push({
      ...
      tone: 'warning',
      title: '还没有订阅源',
      description: '先添加一个订阅源，TSub 才能获取节点并生成可用链接。',
      actionLabel: '添加订阅源',
  ...
  if (disabledSubscriptions.length > 0 && enabledSubscriptions.length > 0) {
    items.push({
      ...
      description: '如需加入默认订阅，请前往订阅源重新启用。',
      actionLabel: '管理订阅',
      actionRoute: '/dashboard/subscriptions',
      actionQuery: { status: 'disabled' },
