...
// [核心修改] saveTsubs 现在接收并发送 profiles
export async function saveTsubs(tsubs, profiles) {
    try {
        // 数据预验证
        ...
        return await api.post('/api/tsubs', { tsubs, profiles });
    } catch (error) {
        return handleApiError(error, 'saveTsubs');
    }
}
...
export async function detectLegacyD1() {
    try {
        ...
        // This optional check commonly gets aborted during the post-login route change.
        return { success: false, error: error?.message || 'Legacy D1 detection unavailable' };
    }
}
...
export async function fetchGithubLatestRelease(repo) {
    try {
        ...
        // Version discovery is optional and is often cancelled during navigation.
        return { success: false, error: error?.message || 'Version check unavailable' };
    }
}
