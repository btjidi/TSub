    ...
    <template v-if="activeTab === 'generator'">
      <section class="grid gap-3 sm:gap-4 xl:gap-5" :class="remoteUpdate ? 'xl:grid-cols-1' : 'xl:grid-cols-[minmax(0,1fr)_360px]'">
        <form class="deployment-form flex min-w-0 flex-col gap-3 sm:gap-4 xl:gap-5" @submit.prevent="requestInstallCommand">
          ...
          <section id="deployment-inbounds-header" data-testid="deployment-inbounds-header" class="deployment-surface order-4 overflow-hidden">
          ...
          <article v-for="(inbound,index) in form.inbounds" :key="inbound.id" data-testid="deployment-inbound-item" class="relative overflow-hidden border-t dark:border-white/10">
            <div data-testid="inbound-basic-row" class="grid grid-cols-[minmax(0,1fr)_minmax(0,0.7fr)_auto_auto] items-end gap-2 p-3 sm:grid-cols-4 lg:grid-cols-[minmax(112px,0.8fr)_minmax(88px,0.45fr)_minmax(176px,1.25fr)_minmax(128px,0.9fr)_minmax(140px,1fr)_auto_auto]">
