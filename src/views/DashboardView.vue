...
import { hasTrafficUsage, resolveEffectiveTrafficTotal } from '../utils/traffic-quota.js';
...
const lastUpdatedTime = computed(() => {
    ...
    return new Date(lastUpdated.value).toLocaleString(locale.value);
});
...
const trafficStats = computed(() => {
    ...
    (subscriptions.value || []).forEach(sub => {
        ...
        if (hasTrafficUsage(sub.userInfo)) {
            totalUsed += (sub.userInfo.upload || 0) + (sub.userInfo.download || 0);
            totalMax += resolveEffectiveTrafficTotal(sub, sub.userInfo);
        }
    });
...
<template>
  <div class="space-y-6">
    ...
    <template v-else>
      <section class="relative overflow-hidden bg-white/85 dark:bg-gray-900/75 border border-gray-100/80 dark:border-white/10 tsub-radius-lg p-4 sm:p-5 shadow-sm">
        ...
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-2 w-full lg:w-auto">
            <button @click="showLogModal = true" class="min-h-11 flex items-center justify-center gap-1.5 px-4 py-2.5 text-sm font-medium bg-white/80 text-gray-700 hover:bg-white dark:bg-white/[0.04] dark:text-gray-300 dark:hover:bg-white/[0.07] tsub-radius-lg transition-colors border border-gray-200/80 dark:border-white/10">
            ...
            <button @click="openBulkImportModal" class="min-h-11 px-4 py-2.5 text-sm font-medium bg-gray-100/80 text-gray-800 hover:bg-gray-200/80 dark:bg-white/[0.04] dark:text-gray-200 dark:hover:bg-white/[0.07] tsub-radius-lg transition-colors border border-transparent dark:border-white/10">
              {{ t('dashboard.actions.bulkImport') }}
            </button>
            <button @click="handleAddProfile" class="min-h-11 px-4 py-2.5 text-sm font-semibold text-white bg-primary-600 hover:bg-primary-700 tsub-radius-lg transition-colors shadow-sm shadow-primary-500/20">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-6">
          <section class="bg-white/90 dark:bg-gray-900/80 p-5 tsub-radius-lg shadow-sm border border-gray-100/80 dark:border-white/10">
            ...
            <div v-if="hasHealthItems" class="grid gap-3">
              <div
                ...
                :key="item.id"
                class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 rounded-[var(--tsub-radius-md)] border p-3"
              ...
              >
                ...
                <div class="flex shrink-0 flex-col sm:flex-row gap-2">
                  <button
                    ...
                    type="button"
                    class="min-h-10 rounded-[var(--tsub-radius-md)] border border-current/20 bg-white/60 px-3 py-2 text-sm font-semibold hover:bg-white/90 dark:bg-white/10 dark:hover:bg-white/15 transition-colors"
                  ...
                  <button
                    ...
                    type="button"
                    class="min-h-10 rounded-[var(--tsub-radius-md)] border border-current/15 px-3 py-2 text-sm font-semibold opacity-80 hover:opacity-100 transition-opacity"
            ...
            <div v-else class="rounded-[var(--tsub-radius-md)] border border-emerald-200/80 bg-emerald-50/80 p-4 text-emerald-700 dark:border-emerald-400/20 dark:bg-emerald-500/10 dark:text-emerald-200">
              <p class="font-semibold">{{ t('dashboard.health.allGood') }}</p>
              <p class="mt-1 text-sm opacity-80">{{ t('dashboard.health.allGoodDesc') }}</p>
            </div>
          </section>

          <section v-if="showFullGuide" class="bg-white/90 dark:bg-gray-900/80 p-5 tsub-radius-lg shadow-sm border border-gray-100/80 dark:border-white/10 min-h-[320px]">
            ...
            <div class="grid md:grid-cols-2 gap-3">
              <div class="p-4 bg-white/80 dark:bg-gray-900/70 tsub-radius-lg border border-gray-200/60 dark:border-white/10">
