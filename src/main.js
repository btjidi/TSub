...
if (typeof window !== 'undefined') {
  console.debug('[TSub] Error Handler Loaded (v2026-01-11-fix-link-error)');
  // 处理未捕获的Promise拒绝
  ...
  // 处理资源加载错误（忽略第三方资源）
  const assetReloadKey = 'tsub:asset-reload';
  ...
  const tryRecoverAssetLoad = async (resourceUrl) => {
    ...
    reloadUrl.searchParams.set('__tsub_reload', Date.now().toString());
...
const loadWebFonts = () => {
  if (document.querySelector('link[data-tsub-web-fonts]')) return;
  const stylesheet = document.createElement('link');
  stylesheet.rel = 'stylesheet';
  stylesheet.href = 'https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600&family=JetBrains+Mono:wght@400;500&display=swap';
  stylesheet.dataset.tsubWebFonts = '';
  document.head.appendChild(stylesheet);
};

if ('requestIdleCallback' in window) {
  window.requestIdleCallback(loadWebFonts, { timeout: 1500 });
} else {
