...
import { deleteDeploymentSource } from '../lib/deployments.js';
...
export function useSubscriptions(markDirty) {
  ...
  const totalRemainingTraffic = computed(() => {
    ...
    return subscriptions.value.reduce((acc, sub) => {
      ...
      if (
        sub.enabled &&
        sub.userInfo &&
        hasTrafficUsage(sub.userInfo) &&
        resolveEffectiveTrafficTotal(sub, sub.userInfo) > 0 &&
        resolveEffectiveTrafficTotal(sub, sub.userInfo) < REASONABLE_TRAFFIC_LIMIT_BYTES
      ) {
        const used = (sub.userInfo.upload || 0) + (sub.userInfo.download || 0);
        const remaining = resolveEffectiveTrafficTotal(sub, sub.userInfo) - used;
      ...
      clearTimeout(timeoutId);
                ...
                if (!result.success) {
                    ...
                    if (subToUpdate.enableNodeCache !== true) {
                        ...
                        if (!isInitialLoad) {
                            markDirty();
                            void dataStore.saveData({ directCommit: dataStore.isDirectCommitMode });
                        }
                    }
  ...
  async function deleteSubscription(subId) {
    const subscription = subscriptions.value.find(item => item.id === subId);
    const deploymentId = subscription?.source?.deploymentId;
    const isManaged = deploymentId && ['tsub-deployment-push', 'tsub-deployment-snapshot'].includes(subscription?.source?.kind);
    if (isManaged) {
      if (!window.confirm(t('subscriptions.deleteManagedConfirm'))) return false;
      try {
        await deleteDeploymentSource(deploymentId);
      } catch (error) {
        showToast(t('subscriptions.deleteManagedFailed'), 'error');
        return false;
      }
    }
    ...
    markDirty();
    if (isManaged) showToast(t('subscriptions.deleteManagedSuccess'), 'success');
    return true;
  }
