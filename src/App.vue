<script setup>
import { defineAsyncComponent, nextTick, onMounted, watch, computed, ref } from 'vue';
...
import { isRouteNavigationLoadingVisible } from './state/routeNavigation.js';
...
const { isDirty, saveState, directCommitFailed, isDirectCommitMode } = storeToRefs(dataStore);
...
const avoidScrollTopBottomAction = computed(() => isLoggedIn.value && [
  '/dashboard/settings',
  '/dashboard/deployments'
].includes(route.path));
const scrollTopBottomActionKind = computed(() => {
  if (!avoidScrollTopBottomAction.value) return '';
  return route.path === '/dashboard/deployments' ? 'deployment' : 'settings';
});

const showModernNavBar = computed(() =>
  isLoggedIn.value || (isSessionLoading.value && route.path.startsWith('/dashboard'))
);
...
const showSavePrompt = computed(() =>
  (isDirty.value && (!isDirectCommitMode.value || directCommitFailed.value))
  || (saveState.value === 'success' && !isDirectCommitMode.value)
);
...
const initialBootPending = ref(true);
const routeRefreshKey = ref(0);
const isRouteRefreshing = ref(false);
const isRouteComponentPending = ref(false);
let bootFinishScheduled = false;

const showRouteLoading = computed(() => (
  isRouteRefreshing.value
  || isRouteNavigationLoadingVisible.value
  || isRouteComponentPending.value
));

const finishInitialBoot = async () => {
  if (!initialBootPending.value || bootFinishScheduled || typeof document === 'undefined') return;
  bootFinishScheduled = true;
  await nextTick();

  window.requestAnimationFrame(() => {
    window.requestAnimationFrame(() => {
      const loader = document.getElementById('tsub-boot-loader');
      if (loader) {
        loader.classList.add('tsub-boot-loading-leave');
        window.setTimeout(() => loader.remove(), 180);
      }
      initialBootPending.value = false;
    });
  });
};

const handleRoutePending = () => {
  isRouteComponentPending.value = true;
};

const handleRouteResolved = () => {
  isRouteComponentPending.value = false;
  finishInitialBoot();
};

const refreshCurrentRoute = async () => {
  if (isRouteRefreshing.value) return;
  isRouteRefreshing.value = true;
  try {
    const minimumLoadingTime = new Promise(resolve => window.setTimeout(resolve, 300));
    await Promise.all([
      isLoggedIn.value ? dataStore.fetchData(true) : Promise.resolve(),
      minimumLoadingTime
    ]);
    routeRefreshKey.value += 1;
  } finally {
    isRouteRefreshing.value = false;
  }
};
...
watch(
  () => [route.fullPath, route.meta?.titleKey, locale.value, sessionStore.publicConfig?.customPage?.enabled, sessionStore.publicConfig?.customPage?.hideBranding],
  () => {
    if (typeof document === 'undefined') return;
    const rawTitle = route.meta?.titleKey ? t(route.meta.titleKey) : '';
    document.title = shouldHidePublicBranding.value
      ? (rawTitle || document.title || '')
      : (rawTitle ? `${rawTitle} - TSub` : 'TSub');
  },
  { immediate: true }
);
...
<template>
  <div :class="theme"
    ...
    <main :class="[
      isCustomPageFullWidth ? 'grow w-full min-h-[100dvh]' : 'grow w-full py-6 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8',
      !isCustomPageFullWidth && (isLoggedIn ? 'pb-56 lg:pb-6' : 'pb-40 lg:pb-6'),
      {
        'flex items-center justify-center': shouldCenterMain,
        'ios-header-padding': showLegacyHeader
      }
    ]">
      <AppLoading v-if="sessionState === 'loading'" />

      <template v-else-if="isLoggedIn">
<!-- Security Banner -->
<div v-if="isDefaultPassword" class="mb-4 px-4 py-4 bg-gradient-to-r from-red-500/90 via-red-600/90 to-red-500/90 backdrop-blur-xl text-white tsub-radius-lg border border-red-400/30 shadow-lg shadow-red-500/20">
...
<button @click="showUpdateNotice = false" class="rounded-md px-2 py-1 text-sm hover:bg-amber-100/80 dark:hover:bg-white/10">{{ t('actions.gotIt') }}</button>
        ...
        <AppLoading v-if="showRouteLoading" />
        <div :class="{ hidden: showRouteLoading }">
        <router-view v-slot="{ Component }">
          <Suspense timeout="0" @pending="handleRoutePending" @resolve="handleRouteResolved">
            <template #default>
              <transition name="fade" mode="out-in">
                <RouteErrorBoundary :reset-key="route.fullPath">
                  <KeepAlive include="SettingsView" :max="2">
      ...
      <template v-else>
        <Suspense timeout="0" @resolve="finishInitialBoot">
...
<Toast />
    ...
    <Footer
      ...
      :class="{ 'hidden lg:block': isLoggedIn }"
      :hide-branding="shouldHidePublicBranding"
    />
<ScrollToTop
  v-if="isLoggedIn || isPublicRoute"
  :avoid-bottom-action="avoidScrollTopBottomAction"
  :bottom-action-kind="scrollTopBottomActionKind"
