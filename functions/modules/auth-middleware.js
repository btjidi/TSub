...
import { getCookieSecret, getAdminCredentialMetadata, getAuthDebugInfo, normalizeAdminUsername, verifyAdminCredentials, JSON_BODY_LIMITS, RequestBodyTooLargeError, readJsonWithLimit } from './utils.js';
import { StorageFactory } from '../storage-adapter.js';
import { clearRateLimit, createRateLimitResponse, inspectRateLimit, recordRateLimitFailure } from './rate-limit.js';
...
export async function getAuthSessionDiagnostic(request, env) {
    ...
    const [timestampText, versionText = '1'] = verifiedData.split(':');
    const ts = parseInt(timestampText, 10);
    const tokenVersion = parseInt(versionText, 10);
    ...
    const credentials = await getAdminCredentialMetadata(env);
    result.verify.versionMatched = Number.isInteger(tokenVersion) && tokenVersion === credentials.authVersion;
    if (!result.verify.versionMatched) {
        result.reason = 'credential_version_mismatch';
        return result;
    }
...
export async function getLoginPasswordDiagnostic(request, env) {
    ...
    const inputUsername = normalizeAdminUsername(payload?.username);
    const inputPassword = typeof payload?.password === 'string' ? payload.password : '';
    result.input.provided = typeof payload?.username === 'string' && typeof payload?.password === 'string';
    result.input.normalizedLength = inputPassword.length;
    result.expected.normalizedLength = 0;
    ...
    result.matched = await verifyAdminCredentials(env, inputUsername, inputPassword);
    result.success = true;
...
export async function handleLogin(request, env) {
    ...
    try {
        ...
        const inputUsername = normalizeAdminUsername(username);
        const limitOptions = {
            scope: 'login',
            identifier: `${request.headers.get('CF-Connecting-IP') || 'unknown'}\0${inputUsername}`,
            limit: 5,
            windowMs: 15 * 60 * 1000,
            blockMs: 15 * 60 * 1000
        };
        const limitState = await inspectRateLimit(request, env, limitOptions);
        if (!limitState.allowed) return createRateLimitResponse(limitState.retryAfter);
        const isPasswordMatched = typeof username === 'string'
            && typeof password === 'string'
            && await verifyAdminCredentials(env, inputUsername, password);

        if (isPasswordMatched) {
            await clearRateLimit(limitState);
            const secret = await getCookieSecret(env);
            const credentials = await getAdminCredentialMetadata(env);
            const token = await createSignedToken(secret, `${Date.now()}:${credentials.authVersion}`);
        ...
        await recordRateLimitFailure(limitState, limitOptions);
