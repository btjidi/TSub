...
import { consumeRateLimit, createRateLimitResponse } from '../rate-limit.js';
...
export async function handleGuestbookPost(request, env) {
    try {
        const hourly = await consumeRateLimit(request, env, { scope: 'guestbook-hour', limit: 5, windowMs: 60 * 60 * 1000 });
        if (!hourly.allowed) return createRateLimitResponse(hourly.retryAfter);
        const daily = await consumeRateLimit(request, env, { scope: 'guestbook-day', limit: 20, windowMs: 24 * 60 * 60 * 1000 });
        if (!daily.allowed) return createRateLimitResponse(daily.retryAfter);
        const storageAdapter = await getStorageAdapter(env);
...
export async function handleGuestbookManageAction(request, env) {
    try {
        ...
        return createErrorResponse(e.status === 413 ? e.message : '操作失败', e.status || 500);
    }
}
