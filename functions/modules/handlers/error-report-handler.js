...
import { consumeRateLimit, createRateLimitResponse, getRateLimitClientIp } from '../rate-limit.js';
import { StorageFactory } from '../../storage-adapter.js';

const ERROR_REPORT_KV_KEY = 'tsub_error_reports';
...
async function getStorage(env) {
    try { return await StorageFactory.getActiveAdapter(env); } catch { return null; }
}
...
export async function handleErrorReportRequest(request, env) {
    ...
    try {
        const rateLimit = await consumeRateLimit(request, env, { scope: 'error-report', limit: 6, windowMs: 60 * 1000 });
        ...
        const report = {
            ...
            clientKey: await crypto.subtle.digest('SHA-256', new TextEncoder().encode(`tsub-report\0${getRateLimitClientIp(request)}`))
                .then(buffer => Array.from(new Uint8Array(buffer), byte => byte.toString(16).padStart(2, '0')).join('').slice(0, 24))
        };

        const raw = await kv.get(ERROR_REPORT_KV_KEY);
        let reports = typeof raw === 'string' ? JSON.parse(raw) : (raw || []);
        ...
        await kv.put(ERROR_REPORT_KV_KEY, reports);
