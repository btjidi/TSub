...
const KV_KEY_CLIENTS = 'tsub_clients_v1';
...
export async function handleClientRequest(request, env) {
    ...
    try {
            ...
            try {
                ...
                if (Array.isArray(data) && data.length > 0) {
                    ...
                    if (migration.updated) {
                        try {
                            await kv.put(KV_KEY_CLIENTS, migration.clients);
                        } catch (writeError) {
        ...
        if (!kv) {
            return createErrorResponse('持久化存储不可用，写操作不可用', 503);
        }

        if (request.method === 'POST') {
            if (path.endsWith('/init')) {
                await kv.put(KV_KEY_CLIENTS, DEFAULT_CLIENTS);
                return createJsonResponse({
                    success: true,
            ...
            let clients = await kv.get(KV_KEY_CLIENTS) || [];
            ...
            await kv.put(KV_KEY_CLIENTS, clients);
        ...
        if (isStorageUnavailableError(e)) {
            return createErrorResponse('持久化存储当前不可用，客户端配置无法保存。', 503);
        }
