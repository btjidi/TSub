...
async function checkRateLimit(userId, env, config) {
    const minute = await consumeRateLimit(null, env, {
        scope: 'telegram-minute', identifier: String(userId),
        limit: config.rate_limit.max_per_minute, windowMs: 60 * 1000
    });
    if (!minute.allowed) {
        return { allowed: false, reason: `操作过快，请1分钟后再试（${config.rate_limit.max_per_minute}/分钟）` };
    }
    const day = await consumeRateLimit(null, env, {
        scope: 'telegram-day', identifier: String(userId),
        limit: config.rate_limit.max_per_day, windowMs: 86400 * 1000
    });
    if (!day.allowed) {
...
async function handleListCommand(chatId, userId, env, page = 0, type = 'all', messageId = null, requestCache = null) {
    try {
        ...
        if (userNodes.length === 0) {
            let emptyMsg = `\uD83D\uDCCB <b>暂无${type === 'sub' ? '订阅源' : (type === 'node' ? '节点' : '资源')}</b>\n\n`; // 📋
            if (type === 'sub') emptyMsg += '发送包含 http/https 的链接即可添加订阅源';
...
async function handleNodeInput(chatId, text, userId, env, requestCache = null) {
    try {
        ...
        for (const url of nodeUrls) {
            ...
            // 注意：TSub 中订阅源也通过 KV_KEY_SUBS 存储
            ...
            : null;
                    ...
                    if (idx >= 0 && idx < subs.length && boundProfileId) {
                        ...
                        if (profile) {
                            ...
                            if (!profile.subscriptions.some(item => (item && typeof item === 'object' ? item.id : item) === sub.id)) {
                    ...
                    if (idx >= 0 && idx < subs.length && boundProfileId) {
                        ...
                        if (profile && profile.subscriptions) {
                            profile.subscriptions = profile.subscriptions.filter(item => (item && typeof item === 'object' ? item.id : item) !== sub.id);
                            await storageAdapter.putAllProfiles(profiles);
                            await answerCallbackQuery(callbackQuery.id, `已从 ${profile.name} 移除`, env);
                            await editTelegramMessage(chatId, messageId,
