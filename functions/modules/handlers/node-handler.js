...
export async function handleNodeCountRequest(request, env) {
    ...
    try {
        ...
        try {
            ...
            if (result.count > 0) {
                const storageAdapter = StorageFactory.createAdapter(env, await StorageFactory.getStorageType(env));
                const originalSubs = await storageAdapter.get('tsub_subscriptions_v1') || [];
                ...
                if (subToUpdate) {
                    if (typeof storageAdapter.updateSubscriptionById === 'function') {
                        ...
                        if (target) {
                            ...
                            await storageAdapter.put('tsub_subscriptions_v1', allSubs);
                        }
                    }
                }
            } else {
...
export async function handleBatchUpdateNodesRequest(request, env) {
    ...
    try {
        ...
        const { subscriptionIds, userAgent = 'TSub-Batch-Update/1.0' } = requestData;
        ...
        const storageAdapter = StorageFactory.createAdapter(env, await StorageFactory.getStorageType(env));
        const allSubscriptions = await storageAdapter.get('tsub_subscriptions_v1') || [];
