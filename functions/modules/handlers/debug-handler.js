    ...
    try {
        ...
        const [allSubscriptions, allProfiles] = await Promise.all([
            typeof storageAdapter.getAllSubscriptions === 'function'
                ...
                : storageAdapter.get('tsub_subscriptions_v1').then(res => res || []),
            typeof storageAdapter.getAllProfiles === 'function'
                ? storageAdapter.getAllProfiles()
                : storageAdapter.get('tsub_profiles_v1').then(res => res || [])
        ]);
...
export async function handleStorageTestRequest(request, env) {
    ...
    try {
        ...
        const testKey = `tsub_test_${Date.now()}`;
        const testValue = {
            timestamp: new Date().toISOString(),
            test: true,
            storageType
...
export async function handleExportDataRequest(request, env) {
    ...
    try {
        ...
        if (includeSubscriptions) {
            const subscriptions = await storageAdapter.get('tsub_subscriptions_v1') || [];
            exportData.data.subscriptions = redactSensitiveObject(subscriptions);
        }

        if (includeProfiles) {
            const profiles = await storageAdapter.get('tsub_profiles_v1') || [];
            exportData.data.profiles = redactSensitiveObject(profiles);
        }

        if (includeSettings) {
            const settings = await storageAdapter.get('tsub_settings_v1') || {};
...
export async function handlePreviewContentRequest(request, env) {
    ...
    try {
        const requestData = await readJsonWithLimit(request, JSON_BODY_LIMITS.normal);
        const { url, userAgent = 'TSub-Preview/1.0', maxLength = 5000 } = requestData;
...
export async function handleTestNotificationRequest(request, env) {
    ...
    try {
        ...
        const storage = StorageFactory.createAdapter(env, await StorageFactory.getStorageType(env));
        const savedSettings = await storage.get(KV_KEY_SETTINGS) || {};
        const settings = { BotToken: botToken || savedSettings.BotToken, ChatID: chatId || savedSettings.ChatID };

        const result = await debugTgNotification(settings, '🔔 <b>通知测试</b> 🔔\n\n这是来自 TSub 的测试消息，用于验证您的配置是否正确。');
