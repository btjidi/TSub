...
import { handleDataRequest, handleTsubsSave, handleSettingsGet, handleSettingsSave, handleSettingsReset, handlePublicProfilesRequest, handlePublicConfig, handleAdminCredentials } from './api-handler.js';
...
import { handleCloudflareEdgePermissionCheck, handleDeployAddressProbe, handleDeployAgentCommandConfig, handleDeployAgentCommandEvents, handleDeployAgentPoll, handleDeployAgentTransferClaim, handleDeployBootstrap, handleDeployCallback, handleDeployPrepare, handleDeployPush, handleDeployQuickTunnelCallback, handleDeployRunScript, handleDeploySubscription, handleDeploymentDefaultsRequest, handleDeploymentsRequest } from './deployment-handler.js';
import { handleDemoDataRequest } from './demo-data-handler.js';
import { getPlatformCapabilities } from '../services/platform-capabilities.js';
import { advanceStorageMigration, getStorageMigration, getStorageStatus, isStorageWriteLocked, startStorageMigration } from '../services/storage-migration-service.js';
import { exportPortablePackage, importPortablePackage } from '../services/portable-transfer.js';
import { handleCloudflareResources, handleCloudflareUsage } from '../services/cloudflare-usage-service.js';

// 常量定义
const OLD_KV_KEY = 'tsub_data_v1';
const KV_KEY_PROFILES = 'tsub_profiles_v1'; // Ensure this is defined if used
...
export async function handleApiRequest(request, env, context = null) {
    ...
    const lockExempt = path === '/login' || path === '/logout' || path.startsWith('/storage/migrations');
    if (writeMethod && !lockExempt) {
        const lock = await isStorageWriteLocked(env);
        if (lock) return createJsonResponse({ success: false, error: 'storage_migration_in_progress', retryAfter: 5 }, 503, { 'Retry-After': '5', 'Cache-Control': 'no-store' });
    }
    ...
    if (path === '/deploy/bootstrap') {
        return handleDeployBootstrap(request, env);
    }

    if (path === '/deploy/edge/quick') return handleDeployQuickTunnelCallback(request, env);

    if (path === '/deploy/run.sh') return handleDeployRunScript(request);
    if (path === '/deploy/prepare') return handleDeployPrepare(request, env);
    if (path.startsWith('/deploy/run/')) return new Response('Not Found', { status: 404, headers: { 'Cache-Control': 'no-store' } });

    if (path === '/deploy/agent/poll') return handleDeployAgentPoll(request, env);
    if (path === '/deploy/agent/transfer/claim') return handleDeployAgentTransferClaim(request, env);
    const agentConfigMatch = path.match(/^\/deploy\/agent\/commands\/([^/]+)\/config$/);
    if (agentConfigMatch) return handleDeployAgentCommandConfig(request, env, decodeURIComponent(agentConfigMatch[1]));
    const agentEventsMatch = path.match(/^\/deploy\/agent\/commands\/([^/]+)\/events$/);
    if (agentEventsMatch) return handleDeployAgentCommandEvents(request, env, decodeURIComponent(agentEventsMatch[1]));

    const addressProbeMatch = path.match(/^\/deploy\/address\/([^/]+)$/);
    if (addressProbeMatch) return handleDeployAddressProbe(request, env, decodeURIComponent(addressProbeMatch[1]));

    if (path === '/deploy/events' || path === '/deploy/callback') {
        return handleDeployCallback(request, env);
    }

    const pushMatch = path.match(/^\/deploy\/push\/([^/]+)$/);
    if (pushMatch) return handleDeployPush(request, env, decodeURIComponent(pushMatch[1]));

    const mirrorMatch = path.match(/^\/deploy\/subscriptions\/([^/]+)\/([^/]+)$/);
    if (mirrorMatch) return handleDeploySubscription(request, env, decodeURIComponent(mirrorMatch[1]), decodeURIComponent(mirrorMatch[2]));
    ...
    if (path === '/data') {
        ...
        return await handleDataRequest(env, context || { env }, request);
    }
    ...
    if (path === '/system/capabilities' && request.method === 'GET') {
        return createJsonResponse({ success: true, data: await getPlatformCapabilities(env) }, 200, { 'Cache-Control': 'no-store' });
    }

    if (path === '/storage/status' && request.method === 'GET') {
        return createJsonResponse({ success: true, data: await getStorageStatus(env) }, 200, { 'Cache-Control': 'no-store' });
    }

    if (path === '/storage/cloudflare/resources' && request.method === 'POST') return handleCloudflareResources(request, env);
    if (path === '/storage/cloudflare/usage' && request.method === 'GET') return handleCloudflareUsage(request, env);
    if (path === '/deployment-edge/cloudflare/check') return handleCloudflareEdgePermissionCheck(request, env);

    if (path === '/backup/portable/export' && request.method === 'POST') {
        try {
            const body = await readJsonWithLimit(request, JSON_BODY_LIMITS.small);
            return createJsonResponse({ success: true, data: await exportPortablePackage(env, body?.passphrase) }, 200, { 'Cache-Control': 'no-store' });
        } catch (error) { return createErrorResponse(error.message, error.status || 400); }
    }

    if (path === '/backup/portable/import' && request.method === 'POST') {
        try {
            const body = await readJsonWithLimit(request, JSON_BODY_LIMITS.portable);
            return createJsonResponse({ success: true, data: await importPortablePackage(env, body?.package, body?.passphrase) }, 200, { 'Cache-Control': 'no-store' });
        } catch (error) { return createErrorResponse(error.message, error.status || 400); }
    }

    if (path === '/storage/migrations' && request.method === 'POST') {
        try {
            const body = await request.json();
            return createJsonResponse({ success: true, data: await startStorageMigration(env, String(body?.target || '')) }, 202);
        } catch (error) { return createErrorResponse(error.message, error.status || 400); }
    }

    const storageMigrationMatch = path.match(/^\/storage\/migrations\/([^/]+)(\/advance)?$/);
    if (storageMigrationMatch) {
        try {
            const id = decodeURIComponent(storageMigrationMatch[1]);
            if (storageMigrationMatch[2] && request.method === 'POST') return createJsonResponse({ success: true, data: await advanceStorageMigration(env, id) });
            if (!storageMigrationMatch[2] && request.method === 'GET') {
                const migration = await getStorageMigration(env, id);
                return migration ? createJsonResponse({ success: true, data: migration }) : createErrorResponse('Migration not found', 404);
            }
            return createErrorResponse('Method Not Allowed', 405);
        } catch (error) { return createErrorResponse(error.message, error.status || 400); }
    }
    ...
    if (path === '/deployments' || path.startsWith('/deployments/')) {
        return handleDeploymentsRequest(request, env, path);
    }

    if (path === '/deployment-defaults') {
        return handleDeploymentDefaultsRequest(request, env);
    }

    if (path === '/demo-data') {
        return handleDemoDataRequest(request, env);
    }
    ...
    if (path === '/kv_test') {
        try {
            ...
            let subsError = null;
            try {
                subsRaw = await kv.get('tsub_subscriptions_v1');
            } catch (e) {
                subsError = e.message;
            }
    ...
    switch (path) {
        case '/tsubs':
            return await handleTsubsSave(request, env);
        ...
        case '/settings/credentials':
            return await handleAdminCredentials(request, env, 'credentials');

        case '/settings/credentials/reset':
            return await handleAdminCredentials(request, env, 'reset');
...
export async function handleSubconverterTestRequest(request, env) {
    ...
    try {
        // 使用公开测试节点内容直接传给后端，避免探测时依赖用户订阅链接或 TSub 回调 URL。
        endpoint.searchParams.set('target', safeTarget);
        endpoint.searchParams.set('url', 'trojan://password@example.com:443?allowInsecure=1&sni=example.com#TSub-Test-Node');
        ...
        const hasUsableOutput = response.ok && /TSub-Test-Node|proxies:|proxy-groups:|trojan/i.test(text);
...
async function handleCronStatusRequest(env) {
    try {
        ...
        try {
            const kv = await StorageFactory.getActiveAdapter(env);
            if (kv) {
                const statusData = await kv.get('cron_last_execution');
                if (statusData) {
                    lastExecution = typeof statusData === 'string' ? JSON.parse(statusData) : statusData;
                    if (lastExecution?.expiresAt && Number(lastExecution.expiresAt) <= Date.now()) lastExecution = null;
                }
            }
        } catch (error) {
...
async function handleCronTriggerRequest(env) {
    try {
        ...
        if (!cronResponse.ok || cronResult?.success === false) {
            return createErrorResponse('Cron execution failed', 502);
        }
        const result = cronResult.summary || {};
            ...
            if (kv) {
                ...
                const executionStatus = {
                    ...
                    result: {
                        ...
                        successfulSyncs: result.updated || 0,
                        failedSyncs: result.failed || 0
                    }
                };
                await kv.put('cron_last_execution', executionStatus);
            }
        } catch (error) {
