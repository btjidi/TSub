...
function buildOutbound(proxy) {
    ...
    const tlsRequested = outbound.tls?.enabled === true || proxy.tls === true || type === 'https' || type === 'socks5-tls';
    if ((proxy['skip-cert-verify'] === true || proxy.skipCertVerify === true) && tlsRequested) {
    ...
    if (proxy.certificatePublicKeySha256 && tlsRequested) {
        outbound.tls = outbound.tls || {};
        outbound.tls.enabled = true;
        outbound.tls.insecure = false;
        outbound.tls.certificate_public_key_sha256 = [normalizeSha256Base64(proxy.certificatePublicKeySha256)];
    }
