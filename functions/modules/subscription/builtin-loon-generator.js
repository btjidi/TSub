...
function clashProxyToLoonResult(proxy) {
    ...
    if (type === 'ss' || type === 'shadowsocks') {
        ...
        parts.push(String(port));
        parts.push(loonQuote(proxy.password || ''));
        parts.push(loonQuote(proxy.uuid || proxy.token || ''));
...
function appendTlsParams(parts, proxy) {
    ...
    if (proxy['skip-cert-verify'] === true || proxy.skipCertVerify === true || proxy.pinnedPeerCertSha256 || proxy.certificatePublicKeySha256) {
