...
import { filterNodesBySelection } from '../utils/node-fingerprint.js';
...
export async function handleProfileMode(request, env, profileId, userAgent, applyTransform = false, skipCertVerify = false) {
    ...
    if (Array.isArray(profileSubIds)) {
        profileSubIds.forEach(item => {
            ...
            const sub = tsubMap.get(id);
            if (sub && sub.enabled && sub.url.startsWith('http')) {
                targetTsubs.push(item && typeof item === 'object' ? { ...sub, ...item } : sub);
            }
        });
    }
    ...
    // 分离HTTP订阅和手工节点
    const targetSubscriptions = targetTsubs.filter(item => item.url.startsWith('http'));
    const targetManualNodes = targetTsubs.filter(item => !item.url.startsWith('http'));
    ...
    // 并行获取HTTP订阅节点
    const subscriptionResults = await Promise.all(targetSubscriptions.map(async sub => {
        const result = await fetchSubscriptionNodes(sub.url, sub.name, userAgent, sub.customUserAgent, false, sub.exclude, sub.fetchProxy, skipCertVerify, Boolean(sub?.plusAsSpace), sub?.enableNodeCache === true);
        if (result?.success && Array.isArray(result.nodes)) result.nodes = await filterNodesBySelection(result.nodes, sub.nodeSelection);
        return result;
    }));
