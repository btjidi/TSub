...
export async function handleExternalPreviewRequest(_request, env, profileId) {
  ...
  const remoteSources = subscriptionIds
    .map(entry => ({ entry, item: byId.get(entry && typeof entry === 'object' ? entry.id : entry) }))
    .filter(({ item }) => item && isRemoteSubscription(item))
    .map(({ entry, item }) => ({ id: item.id, type: 'subscription', name: item.name || '', url: item.url || '', nodeSelection: entry?.nodeSelection || null }));
