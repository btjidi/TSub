...
export async function checkAndNotify(sub, settings, env) {
    const effectiveUserInfo = resolveEffectiveUserInfo(sub, sub.userInfo);
    if (!effectiveUserInfo) return;

    const ONE_DAY_MS = 24 * 60 * 60 * 1000;
    const now = Date.now();

    // 1. 检查订阅到期
    if (effectiveUserInfo.expire) {
        const expiryDate = new Date(effectiveUserInfo.expire * 1000);
...
<b>到期日期:</b> <code>${expiryDate.toLocaleDateString('zh-CN')}</code>`;
    ...
    // 2. 检查流量使用
    const { upload, download, total } = effectiveUserInfo;
    if (total > 0 && hasTrafficUsage(effectiveUserInfo)) {
