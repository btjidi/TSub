...
export const BUILTIN_PROTOCOL_DEFAULTS = Object.freeze({
  ...
  naive: { transport: 'https', outbound: 'direct', tlsMode: 'tls', serverName: '', path: '/', serviceName: 'tsub' }
});
...
export function resolveV2Config(raw = {}, systemDefaults = {}, context = {}) {
  ...
  const resolvedInbounds = inboundSources.map((source, index) => {
    ...
    const serverName = text(tlsSource.serverName || protocolDefaults.serverName || (tlsMode === 'tls' ? 'tsub.local' : ''), 253);
...
function compileInboundNode(config, item, entry, edge = false) {
    ...
    if (item.protocol === 'vmess') {
      ...
      const payload = { v: '2', ps: decodeURIComponent(name), add: rawHost, port: String(portValue), id: item.credentials.uuid, aid: '0', scy: config.runtime.core === 'sing-box' ? 'none' : 'auto', net: item.transport, type: 'none', host: transportHost, path: item.transport === 'grpc' ? item.transportOptions.serviceName : item.transportOptions.path, tls: tlsMode === 'tls' || tlsMode === 'reality' ? 'tls' : '', sni: tlsServerName, alpn: tlsMode === 'reality' ? 'h2' : undefined, fp: tlsMode === 'reality' ? 'chrome' : undefined, pbk: tlsMode === 'reality' ? realityPublicKey(item) : undefined, sid: tlsMode === 'reality' ? item.tls.shortId : undefined, allowInsecure: pinned || item.tls.insecure ? true : undefined, insecure: pinned || item.tls.insecure ? true : undefined, pcs: pinned ? '__TSUB_CERT_PIN_SHA256__' : undefined, spki: pinned ? '__TSUB_CERT_SPKI_SHA256__' : undefined };
