...
function validateExternalQuota(payload) {
  if (!Object.prototype.hasOwnProperty.call(payload || {}, 'trafficQuotaOverrideBytes')) return { valid: true, value: undefined };
  return validateTrafficQuotaOverride(payload.trafficQuotaOverrideBytes);
}
...
async function removeSubscriptionIdFromProfiles(storageAdapter, id) {
  ...
  for (const profile of profiles) {
    ...
    if (!subscriptions.some(item => (item && typeof item === 'object' ? item.id : item) === id)) continue;
    const updated = { ...profile, subscriptions: subscriptions.filter(item => (item && typeof item === 'object' ? item.id : item) !== id), updatedAt: nowIso() };
...
function createNodeCountRequest(body) {
  return new Request('https://tsub.internal/api/node_count', {
    method: 'POST',
...
export async function inspectRemoteSubscription(env, payload = {}) {
  ...
  const effectiveUserAgent = runtime.userAgent || getProcessedUserAgent('TSub-External-Validate/1.0', requestedUrl);
...
export async function handleExternalSubscriptionsRequest(request, env, selector = null) {
  ...
  if (request.method === 'POST' && !id) {
    ...
    const quota = validateExternalQuota(payload);
    if (!quota.valid) return createExternalError('invalid_traffic_quota', 'trafficQuotaOverrideBytes must be a positive safe integer or null', 400);
    ...
    if (quota.value !== undefined) item.trafficQuotaOverrideBytes = quota.value;
  ...
  if (request.method === 'PATCH' && id) {
    ...
    const quota = validateExternalQuota(payload);
    if (!quota.valid) return createExternalError('invalid_traffic_quota', 'trafficQuotaOverrideBytes must be a positive safe integer or null', 400);
    if (MANAGED_SUBSCRIPTION_KINDS.has(current?.source?.kind)) {
      const immutableFields = ['name', 'url', 'group', 'tags', 'userAgent', 'proxy'];
      if (immutableFields.some(key => payload[key] !== undefined)) {
        return createExternalError('managed_subscription_readonly', 'Managed subscription system fields are read-only', 409);
      }
    }
    const updated = {
      ...current,
      ...
      ...(payload.exclude !== undefined ? { exclude: String(payload.exclude || '') } : {}),
      ...(payload.excludeTraffic !== undefined ? { excludeTraffic: payload.excludeTraffic === true } : {}),
      ...(payload.website !== undefined ? { website: String(payload.website || '').trim() } : {}),
      ...(payload.notes !== undefined ? { notes: String(payload.notes || '') } : {}),
      ...(quota.value !== undefined ? { trafficQuotaOverrideBytes: quota.value } : {}),
      updatedAt: nowIso()
    };
  ...
  if (request.method === 'DELETE' && id) {
    ...
    if (!current) return createExternalError('subscription_not_found', 'Subscription not found', 404);
    if (MANAGED_SUBSCRIPTION_KINDS.has(current?.source?.kind)) {
      return createExternalError('managed_subscription_readonly', 'Managed subscriptions must be removed through Proxy Deployment', 409);
    }
