...
import { filterNodesBySelection } from '../modules/utils/node-fingerprint.js';
...
async function writeSubscriptionRuntimeInfo(storage, sub, runtimeInfo = {}) {
    ...
    try {
        ...
        if (typeof storage.get === 'function' && typeof storage.put === 'function') {
            const all = await storage.get('tsub_subscriptions_v1');
 ...
 */
export async function generateCombinedNodeList(context, config, userAgent, tsubs, prependedContent = '', profilePrefixSettings = null, debug = false, skipCertVerify = false) {
// 判断是否启用手动节点前缀
...
const prependGroupName = profilePrefixSettings?.prependGroupName ?? false;
    ...
    // 重构后的手动节点处理逻辑
    const manualSubSourceGroups = tsubs.filter(sub => {
        const url = typeof sub?.url === 'string' ? sub.url.trim() : '';
    ...
    const httpSubs = tsubs.filter(sub => sub && sub.url && sub.url.toLowerCase().startsWith('http'));
    ...
    const fetchSingleSubscription = async (sub) => {
        ...
        const managedDeploymentSnapshot = ['tsub-deployment-push', 'tsub-deployment-snapshot'].includes(sub?.source?.kind);
        ...
        try {
            ...
            // Profile 子集必须在任何重命名、前缀和协议渲染前过滤。
            validNodes = await filterNodesBySelection(validNodes, sub.nodeSelection);
            ...
            if (cacheEnabled && !managedDeploymentSnapshot) {
    ...
    try {
        ...
        const isInternalRequest = userAgent.includes('TSub-Backend') || userAgent.includes('TelegramBot');
