    ...
    try {
        ...
        if (type === 'vmess') {
            ...
            const vmessConfig = {
                ...
                pcs: proxy.pinnedPeerCertSha256 || proxy['pinned-peer-cert-sha256'] || proxy.certSha256 || ''
            };
        ...
        if (type === 'trojan') {
            ...
            if (pinnedPeerCertSha256) params.push(`pcs=${encodeURIComponent(pinnedPeerCertSha256)}`);
            else if (proxy.skipCertVerify || proxy['skip-cert-verify']) params.push('allowInsecure=1');
        ...
        if (type === 'hysteria2' || type === 'hy2' || type === 'hy') {
            ...
            else if (proxy.skipCertVerify || proxy['skip-cert-verify']) params.push('insecure=1');
        ...
        if (type === 'anytls') {
            ...
            else if (proxy['skip-cert-verify']) params.push('insecure=1');
            if (proxy.padding !== undefined) params.push(`padding=${proxy.padding}`);
            const query = params.length > 0 ? `?${params.join('&')}` : '';
