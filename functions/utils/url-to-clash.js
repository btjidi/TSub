...
function parseVlessUrl(url) {
    try {
        ...
        const pinnedPeerCertSha256 = params.get('pcs') || params.get('pinnedPeerCertSha256');
        if (pinnedPeerCertSha256) proxy.pinnedPeerCertSha256 = pinnedPeerCertSha256;
        if (params.get('spki')) proxy.certificatePublicKeySha256 = params.get('spki');
...
function parseAnytlsUrl(url) {
    try {
        ...
        const pinnedPeerCertSha256 = params.get('pcs')
            || params.get('pinnedPeerCertSha256')
            || params.get('pinned-peer-cert-sha256')
