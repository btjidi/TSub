...
function isUselessNode(record) {
    ...
    if (
        ['trojan', 'vmess'].includes(protocol)
        && server === '127.0.0.1'
        && /(?:^(?:↑|↓|TOT|REM|上|下|总|剩|上行|下行|总计|剩余|上行流量|下行流量|总计流量|剩余流量)\s|流量剩余|订阅已失效|订阅已到期|已过期|到期提醒|到期时间|套餐到期|过期时间)/i.test(name)
    ) {
        return true;
    }

    return /(?:流量剩余|剩余流量|上行流量|下行流量|总计流量|已用流量|总流量|套餐到期|到期时间|过期时间|订阅已失效|订阅已到期|已过期|官网|群组|频道|联系客服|测试节点|回车更新|点击订阅|剩余套餐|订阅信息)/i.test(name);
}
