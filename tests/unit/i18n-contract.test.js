...
describe('TSub i18n contract', () => {
  ...
  it('uses server terminology in user-visible translations', () => {
    const strings = value => Object.values(value).flatMap(item => typeof item === 'string' ? [item] : strings(item));
    expect(strings(messages).filter(value => /\bVPS\b/.test(value))).toEqual([]);
    expect(t('deployments.generateDeploymentCommand')).toBe('生成部署命令');
    expect(t('deployments.oneTimeCommand')).toBe('一次性部署命令');
    expect(t('deployments.commandPlaceholder')).toContain('目标服务器');
    expect(t('deployments.oneTimeCommand', {}, 'en-US')).toBe('One-time Deployment Command');
  });
  ...
  it('detects locale from persisted preference before browser language', () => {
    localStorage.setItem('tsub:locale', 'en-US');
  ...
  it('installs a global translator and reactive locale switcher', async () => {
    ...
    expect(localStorage.getItem('tsub:locale')).toBe('en-US');
