...
function clearExternalNodesMemoryCache() {
    delete globalThis.__tsubExternalNodesCallbackCache;
}
...
describe('external nodes callback service', () => {
    ...
    it('reuses existing temporary KV object instead of writing again', async () => {
        ...
        const result = await putExternalNodes({ env: { TSUB_KV: kv }, profileId: 'profile-a', nodesText: 'ss://cached#A\n' });
    ...
    it('builds a public callback url without leaking the cache key', async () => {
        ...
        const callbackUrl = buildExternalNodesCallbackUrl('https://tsub.example/stable-token?target=clash&refresh=1', token);

        expect(callbackUrl).toBe('https://tsub.example/api/external-nodes-callback?token=payload.signature');
    ...
    it('serves cached callback nodes as plain text and returns 410 when cache is missing', async () => {
        ...
        const response = await handleExternalNodesCallbackRequest(
            new Request(`https://tsub.example/api/external-nodes-callback?token=${encodeURIComponent(token)}`),
            { TSUB_KV: kv, CALLBACK_TOKEN_SECRET: secret }
        );
        ...
        const missingResponse = await handleExternalNodesCallbackRequest(
            new Request(`https://tsub.example/api/external-nodes-callback?token=${encodeURIComponent(token)}`),
            { TSUB_KV: { get: vi.fn(async () => null), put: vi.fn(async () => undefined), delete: vi.fn(async () => undefined) }, CALLBACK_TOKEN_SECRET: secret }
        );
    ...
    it('can serve cached callback nodes as base64 for subconverter remote URL fetches', async () => {
        ...
        const response = await handleExternalNodesCallbackRequest(
            new Request('https://tsub.example/api/external-nodes-callback?token=' + token + '&encoding=base64'),
            { TSUB_KV: kv, CALLBACK_TOKEN_SECRET: secret }
        );

        expect(response.status).toBe(200);
        expect(response.headers.get('X-TSub-Callback-Encoding')).toBe('base64');
    ...
    it('uses D1 as durable callback node storage when KV is unavailable', async () => {
        ...
        const env = { TSUB_DB: createMemoryD1(), CALLBACK_TOKEN_SECRET: secret };
        ...
        clearExternalNodesMemoryCache();
        const response = await handleExternalNodesCallbackRequest(
            new Request(`https://tsub.example/api/external-nodes-callback?token=${token}`),
            env
        );
