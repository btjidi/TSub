...
import { handleTsubRequest } from '../../functions/modules/subscription/main-handler.js';
  ...
  return {
    prepare(sql) {
      return {
        ...
        async first() {
          if (/FROM storage_control/i.test(sql)) {
            return activeStorage ? { active_storage: activeStorage, state: 'idle', epoch: 1 } : null;
          }
          if (/FROM settings/i.test(sql) && this.queryValue === 'worker_settings_v1') {
            return null;
          }
          if (/FROM settings/i.test(sql) && this.queryValue === 'main' && settings) {
        ...
        async run() {
          if (/INSERT INTO storage_control/i.test(sql)) {
            activeStorage = this.queryValue === 'main' ? 'd1' : this.queryValue;
          }
          return { success: true };
        }
      };
...
describe('security hardening', () => {
  ...
  it('rejects oversized login JSON bodies before parsing', async () => {
    const env = { TSUB_KV: createKv(), COOKIE_SECRET: 'stable-cookie-secret' };
    const response = await handleLogin(new Request('https://example.com/api/login', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ username: 'admin', password: 'admin', padding: 'x'.repeat(32 * 1024) })
    }), env);
  ...
  it('does not expose public auth debug endpoints in production', async () => {
    const env = { TSUB_KV: createKv(), COOKIE_SECRET: 'stable-cookie-secret', ADMIN_PASSWORD: 'secret-password' };
  ...
  it('supports cron query secret compatibility and Authorization bearer', async () => {
    ...
    const env = {
      ASSETS: createAssets(),
      TSUB_DB: createD1({ cronSecret: 'cron-secret' })
    };
  ...
  it('returns 404 for retired cron function paths instead of serving the SPA', async () => {
    const assetFetch = vi.fn(async () => new Response('<html>spa</html>', {
      headers: { 'Content-Type': 'text/html' }
    }));

    for (const path of ['/_cron.js', '/_schedule.js', '/cron-dashboard.js']) {
      const response = await onRequest({
        request: new Request(`https://example.com${path}`),
        env: { TSUB_KV: createKv(), ASSETS: { fetch: assetFetch } }
      });
  ...
  it('serves extensionless TSub Proxy providers publicly without subscription-token routing', async () => {
    const assetFetch = vi.fn(async request => new Response('busybox-binary', {
      headers: { 'Content-Type': 'application/octet-stream', 'Cache-Control': 'public, max-age=14400' }
    }));
    const response = await onRequest({
      request: new Request('https://example.com/proxy/v2/providers/busybox-1.38.0-musl-amd64'),
      env: { TSUB_KV: createKv(), ASSETS: { fetch: assetFetch } }
    });

    expect(response.status).toBe(200);
    expect(await response.text()).toBe('busybox-binary');
  ...
  it('tests subconverter backends with a synthetic node without exposing user subscriptions', async () => {
    ...
    expect(testUrl.searchParams.get('url')).toContain('TSub-Test-Node');
  ...
  it('masks subscription token in parse logs', async () => {
    ...
    const env = {
      TSUB_KV: createKv({
        tsub_settings_v1: JSON.stringify({
          mytoken: 'super-secret-token',
          subConverter: 'backend',
          selectedRules: [],
          customRules: []
        }),
        tsub_subscriptions_v1: JSON.stringify([]),
        tsub_profiles_v1: JSON.stringify([])
      })
    };

    await handleTsubRequest({
  ...
  it('redacts sensitive fields from exported settings and subscriptions', async () => {
    ...
    expect(body.exportData.data.settings.safeName).toBe('TSub');
  ...
  it('enables subscription fetch diagnostics only through an explicit debug flag', async () => {
    vi.useFakeTimers();
    try {
      const fetchSpy = vi.spyOn(globalThis, 'fetch').mockRejectedValue(new Error('upstream unavailable'));
      const debugSpy = vi.spyOn(console, 'debug').mockImplementation(() => {});
      const url = 'https://airport.example/sub/token-shaped-path';

      const normalRequest = fetchSubscriptionNodes(url, 'Airport', 'TSub-Test', null, false);
      await vi.runAllTimersAsync();
      await normalRequest;
      expect(fetchSpy).toHaveBeenCalled();
      expect(debugSpy).not.toHaveBeenCalled();

      const debugRequest = fetchSubscriptionNodes(url, 'Airport', 'TSub-Test', null, true);
      await vi.runAllTimersAsync();
      await debugRequest;
