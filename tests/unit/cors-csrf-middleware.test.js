...
import { corsMiddleware, csrfOriginMiddleware, securityHeadersMiddleware } from '../../functions/middleware/cors.js';
...
describe('CORS and CSRF middleware hardening', () => {
  it('preserves stricter response-specific security headers', async () => {
    const response = await securityHeadersMiddleware(
      makeMiddlewareRequest('https://example.com/api/deploy/run.sh'),
      async () => new Response('ok', { headers: { 'Referrer-Policy': 'no-referrer' } })
    );
  ...
  it('centralizes CORS headers without wildcard defaults in JSON helpers', async () => {
    ...
    const helperResponse = await handleApiRequest(new Request('https://example.com/api/public_config'), { TSUB_KV: createKv() });
    ...
    const cookieWithoutOrigin = await csrfOriginMiddleware(
      {
        ...
        headers: {
          get(name) {
            return String(name || '').toLowerCase() === 'cookie' ? 'tsub_session=[REDACTED]' : null;
          }
        }
      },
      async () => new Response('should-not-run'),
