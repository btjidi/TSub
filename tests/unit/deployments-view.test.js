...
describe('TSub Proxy simplified deployment generator', () => {
  ...
  it('shows common global and inbound fields initially while keeping advanced overrides collapsed', async () => {
    ...
    expect(inboundGrid).toContain('minmax(112px,0.8fr)');
    expect(inboundGrid).toContain('minmax(88px,0.45fr)');
    ...
    expect(advanced.classes()).toEqual(expect.arrayContaining(['w-fit', 'shrink-0', 'justify-self-end', 'whitespace-nowrap', 'px-2', 'sm:col-start-4']));
    expect(advanced.classes()).not.toContain('min-w-[4.75rem]');
    expect(wrapper.get('[data-testid="inbound-delete-0"]').classes()).toEqual(expect.arrayContaining(['w-10', 'p-0', 'sm:col-start-5']));
    expect(wrapper.get('[data-testid="vps-subscription-token-field"]').classes()).toEqual(expect.arrayContaining(['col-span-2', 'sm:col-span-1', 'lg:col-span-2']));
