...
function mountPanel(props = {}) {
  return mount(SubscriptionPanel, {
    ...
    global: {
      ...
      stubs: {
        Card: { props: ['tsub'], template: '<article class="card-stub">{{ tsub.name }}</article>' },
...
describe('SubscriptionPanel UX', () => {
  it('keeps the panel header focused on local actions instead of duplicating dashboard statistics', () => {
    ...
    expect(wrapper.text()).toContain('维护订阅源，添加/导入后可在卡片中查看节点、流量与到期信息。');
  ...
  it('exposes a dedicated file import action when enabled', async () => {
    const wrapper = mountPanel({ enableFileImport: true });
    expect(wrapper.text()).toContain('文件导入');

    const input = wrapper.get('[data-testid="subscription-file-input"]');
    Object.defineProperty(input.element, 'files', {
      configurable: true,
      value: [new File(['https://example.com/sub'], 'subscriptions.txt', { type: 'text/plain' })]
    });
    await input.trigger('change');

    expect(wrapper.emitted('importFile')).toHaveLength(1);
    expect(wrapper.emitted('importFile')[0][0].name).toBe('subscriptions.txt');
    const bulk = wrapper.findAll('button').find(button => button.text() === '批量导入');
    const file = wrapper.get('[data-testid="subscription-file-import"]');
    expect(bulk.element.compareDocumentPosition(file.element) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
    expect(file.find('svg').exists()).toBe(false);
  });

  it('offers direct add and bulk import actions in the empty state', async () => {
    const wrapper = mountPanel();

    expect(wrapper.text()).toContain('添加订阅源');
