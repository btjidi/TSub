...
describe('LogService storage selection', () => {
  ...
  it('writes access logs into D1-backed storage when storageType is d1', async () => {
    ...
    expect(Array.from(d1Storage.store.keys())).toContain('tsub_system_logs:index');
    expect(Array.from(kvStorage.store.keys())).not.toContain('tsub_system_logs:index');

    const logs = await LogService.getLogs({ TSUB_DB: {}, TSUB_KV: {} });
  ...
  it('clears both D1 logs and legacy KV log buckets in d1 mode', async () => {
    ...
    d1Storage.store.set('tsub_system_logs:index', ['tsub_system_logs:2026-04-15']);
    d1Storage.store.set('tsub_system_logs:2026-04-15', [{ id: 'd1-log', timestamp: Date.now() }]);

    kvStorage.store.set('tsub_system_logs:index', ['tsub_system_logs:2026-04-14']);
    kvStorage.store.set('tsub_system_logs:2026-04-14', [{ id: 'kv-log', timestamp: Date.now() }]);

    createAdapter.mockImplementation((env, type) => (type === 'd1' ? d1Storage : kvStorage));

    const { LogService } = await import('../../functions/services/log-service.js');

    const result = await LogService.clearLogs({ TSUB_DB: {}, TSUB_KV: {} });
