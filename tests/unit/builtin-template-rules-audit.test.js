...
describe('Builtin template rule audit', () => {
    it('keeps exactly one builtin template marked as default and uses TSub minimal as that default', () => {
        ...
        expect(builtinDefaultAssets[0].url).toBe('builtin:clash_tsub_minimal');
    });

    it('uses a consistent manual switch group name in TSub minimal', () => {
        const model = getOptimizedTemplateModel('clash_tsub_minimal');
    ...
    it('does not put TSub media AI main selector inside itself', () => {
        const model = getOptimizedTemplateModel('clash_tsub_media_ai');
        const mainGroup = findGroup(model, '🚀 节点选择');
