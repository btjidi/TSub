...
`, { status: 200 })));
    ...
    it('renders remote Clash YAML profile templates by injecting TSub nodes locally', async () => {
        vi.stubGlobal('fetch', vi.fn(async () => new Response(`
mode: rule
mixed-port: 7890
proxy-groups:
...
`, { status: 200 })));
        ...
        expect(result.contentType).toBe('application/x-yaml; charset=utf-8');
        expect(result.headers['X-TSub-Template-Mode']).toBe('clash-yaml-profile');
