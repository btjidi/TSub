...
describe('external api route boundaries', () => {
  it('returns 401 when bearer token is missing', async () => {
    const env = createEnv({
      externalApi: { enabled: true, tokens: [{ name: 'default', token: 'tsub_ok' }] }
    });
  ...
  it('returns 403 when external api is disabled', async () => {
    const env = createEnv({
      externalApi: { enabled: false, tokens: [{ name: 'default', token: 'tsub_ok' }] }
    });

    const response = await handleApiRequest(createRequest('/subscriptions', { token: 'tsub_ok' }), env);
    const body = await response.json();
  ...
  it('returns 404 for unknown external routes', async () => {
    ...
    const response = await handleApiRequest(createRequest('/unknown', { token: 'tsub_ok' }), env);
    const body = await response.json();
  ...
  it('returns 405 for unsupported methods', async () => {
    ...
    const response = await handleApiRequest(createRequest('/subscriptions', { method: 'PUT', token: 'tsub_ok' } ), env);
    const body = await response.json();
