...
describe('useSettingsLogic externalApi normalization', () => {
  ...
  it('blocks enabled empty custom traffic labels before the save request', async () => {
    const { settings, handleSave } = useSettingsLogic();
    settings.value.storageType = 'kv';
    settings.value.trafficNodeDisplay = {
      upload: { enabled: true, label: 'custom', customLabel: '   ' }
    };
    ...
    expect(showToast).toHaveBeenCalledWith('上行流量的自定义名称不能为空', 'error');
  });
});
